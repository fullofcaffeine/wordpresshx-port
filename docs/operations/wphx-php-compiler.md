...
The runner emits `build/wp-core/wphx-comp-php-group-wp-http/generated/wp-includes/class-wp-http.php` with `WP_Http::processResponse`, `WP_Http::chunkTransferDecode`, protected `WP_Http::parse_url`, `WP_Http::buildCookieHeader( &$r )`, `WP_Http::processHeaders( $headers, $url = '' )`, `WP_Http::is_ip_address( $maybe_ip )`, and `WP_Http::browser_redirect_compatibility( ..., &$options, $original )` in a single generated `WP_Http` class. It lints the generated PHP, verifies there is exactly one `class WP_Http`, verifies the WordPress non-throwing bootstrap profile, checks reflection-visible by-reference/default/protected method ABI, compares oracle/candidate behavior for all seven helper cases, confirms parser delegation, native-array header/cookie IR usage, direct IP helper delegation, object-property reads, class constants, and by-reference redirect option mutation, and records `class:WP_Http` with `unsupported=[]`. It does not claim whole-file `WP_Http`, `WP_Http::request`, live HTTP transport, installed distribution behavior, generated `WP_Http_Cookie`, or mixed-template ownership.
