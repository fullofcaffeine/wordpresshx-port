...
`WPHX-COMP-PHP-TRANSPORT-RAW-BLOCK-RETIREMENT` removes the remaining inline raw PHP adapter bodies from the WordPress profile. `WP_Http::_get_first_available_transport()` still renders from a compiler-owned template under `src/wphx/compiler/php/templates/wordpress/`, and the WPHX-312.66 runner requires path, SHA-256, placeholder, ownership, and upstream-reference evidence before accepting that generated shell body. `WPHX-COMP-PHP-DISPATCH-ADAPTER-IR-PROMOTION` then promotes private `WP_Http::_dispatch_request()` from a compiler-owned template into structured WPHX PHP core IR using `PhpStaticLocal` and dynamic class construction. The profile still emits `PhpRawBlock(rendered.code)` as the final narrow target-native boundary for remaining templates, but `_dispatch_request` is no longer part of that raw boundary.
