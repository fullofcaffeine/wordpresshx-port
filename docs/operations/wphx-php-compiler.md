...
Source-map evidence uses `canonical-source-map-v1`: runners parse the map, preserve source order, mappings, names, and source content, replace only repository/Haxe-standard/haxelib roots with stable logical roots, sort JSON object keys, and hash the canonical bytes. Records prove that the raw map artifact exists, but do not commit raw hashes or byte counts that change with the absolute checkout path. Run `npm run evidence:canonical-source-map:check` to prove equivalent maps from distinct checkout roots produce identical records while mapping or source-content changes still change the digest.
...
`WPHX-COMP-PHP-USABLE` has passed the bounded usable-compiler gate for resuming parallel Core PHP port slices. Its gates are gap inventory, runtime/std strategy, reusable core lowering, a small whole-file WordPress pilot, and adoption CI. The profile/core promotion audit is now a continuous amber-condition gate inside adoption CI. Parallel Core slices may proceed when they use WPHX PHP for new public WordPress PHP emission or intentionally drive a named compiler gap while keeping the adoption gate green.
...
The profile/core promotion audit is the continuous profile-accretion gate:

```bash
npm run wphx:php:profile-core-promotion-audit
...
The native-array core fixture is:
...
It records `manifests/wphx-php/native-array-mutation-core.v1.json` and `receipts/compiler/wphx-comp-php-native-array-mutation-core.v1.json`. The fixture emits `wp-includes/wphx-native-array-mutation.php` from ordinary typed Haxe bodies with no `@:wp.adapter`, `@:wp.haxeHelper`, or `@:wp.haxeBootstrap` bridge. It proves typed native array indexed reads/writes, nested indexed mutation, associative fallback reads, keyed writes, appends, unset, `array_key_exists`, `isset`, and `empty` lowering, plus PHP distinctions for null, false, `0`, `'0'`, empty string, and missing keys. The fixture records `unsupported=[]` and `core_ir_features=["typed.expr.anonymous-object-field","typed.expr.anonymous-object-literal","typed.expr.array-access","typed.expr.array-empty","typed.expr.array-fallback-read","typed.expr.array-isset","typed.expr.array-key-exists","typed.expr.array-literal","typed.stmt.array-append","typed.stmt.array-unset","typed.stmt.array-write"]`. By-reference array slot aliasing and the full PHP key-coercion/sparse/sort/serialization matrix remain deferred pressure.

The callable/closure core fixture is:
...
It records `manifests/wphx-php/callable-closure-core.v1.json` and `receipts/compiler/wphx-comp-php-callable-closure-core.v1.json`. The fixture emits `wp-includes/wphx-callable-closure.php` from ordinary typed Haxe bodies with no `@:wp.adapter`, `@:wp.haxeHelper`, or `@:wp.haxeBootstrap` bridge. It proves anonymous Haxe functions lowered to PHP `static function` closures, static class/method callable arrays, `call_user_func`, `call_user_func_array`, accepted-args slicing through `array_slice`, and a by-reference callback payload mutation through `call_user_func_array(..., [&$payload])`. The fixture records `unsupported=[]` and `core_ir_features=["expr.reference","expr.static-closure","typed.expr.accepted-args-slice","typed.expr.anonymous-object-field","typed.expr.anonymous-object-literal","typed.expr.array-literal","typed.expr.call-user-func","typed.expr.call-user-func-array","typed.expr.callable-array","typed.expr.reference-array","typed.expr.static-closure"]`. Arbitrary callable shape validation, string callbacks, object/method tuples, invokable objects, closure captures, by-reference closure parameters, and full hook-kernel ownership remain deferred pressure.
...
The HTTP/embed/oEmbed file-ownership inventory gate is:
...
It records `manifests/wphx-php/file-ownership-inventories.v1.json` and `receipts/compiler/wphx-comp-php-file-ownership-inventories.v1.json`. The runner tokenizes the WordPress 7.0 oracle files `wp-includes/class-wp-http.php`, `wp-includes/class-wp-embed.php`, and `wp-includes/class-wp-oembed.php` with PHP's tokenizer, inventories 3 classes, 46 methods, 10 properties, top-level guards/includes/effects, properties/defaults, visibility, references, globals/superglobals, hooks, callbacks, mixed PHP/HTML, Requests integration, remote fetch/discovery, JSON/XML parsing, and the current WPHX generated-boundary evidence. Each file gets a whole-file ownership checklist covering source-surface completion, unsupported=[], deterministic original-path emission, all-symbol reflection/ABI checks, broad oracle/candidate behavior, packaged installed gates, plugin/theme reflection, stack traces/source maps, readability, and selected upstream PHPUnit pass/pass ratchets. The manifest explicitly keeps the current `class-wp-http.php`, `class-wp-embed.php`, and `class-wp-oembed.php` evidence at selected-boundary status; it does not claim full file ownership.

The file-segment core API fixture is:
...
npm run wphx:php:gap-inventory
npm run wphx:php:gap-inventory:check
npm run evidence:canonical-source-map:check
npm run wphx:php:runtime-stdlib-strategy
npm run wphx:php:runtime-stdlib-strategy:check
npm run wphx:php:core-lowering-pilot
npm run wphx:php:core-lowering-pilot:check
...
- `@:wp.scriptAdapter("adapter-name")` emits a bounded direct file-scope, minimized whole-file, or segment-shell script adapter selected through the file-segment plan registry. The current adapters are `include-side-effects`, which proves include timing, include returns, function-scope locals, and output buffering; `deprecated-class-http`, which proves the minimized whole-file `class-http.php` shim; `template-segment-admin-style`, which proves the first minimized generated segment shell with ordered guard/declaration/script/literal/template/control/return behavior and now prints through the bounded segment-plan printer; and `template-segment-nested-parent` / `template-segment-nested-partial`, which prove a generated parent/partial include pair with caller-scope locals, include returns, repeated include, and include_once behavior through the same segment-plan printer.
