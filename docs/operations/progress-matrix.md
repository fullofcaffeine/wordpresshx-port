...
Source checkpoint: `WPHX-307.04 post metadata/cache adapter-contract candidate`
Tracking issue: `WPHX-000.02`

## Super Progress

Current super progress toward "WordPress completely in Haxe and functional": **39.2%**.
...
| WordPress PHP runtime Haxe ownership | 40 | 21 | 8.4 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, user/auth and posts/query Haxe adapter-contract candidates, auth-domain oracle fixtures, and WPHX-307 posts/query, post CRUD/status, and post metadata/cache adapter-contract evidence, but much of Core remains unported |
...
| `WPHX-300` | WordPress PHP core | open | 8/8 original direct children closed; WPHX-306 is closed; WPHX-307 is active; WPHX-308 through WPHX-320 remain seeded follow-up domains | Bootstrap, hooks, error/formatting/escaping/KSES, options/cache, wpdb, REST, multisite, first-party PHP manifest/split closure, users/auth adapter-contract/domain-closure evidence, posts/query surface inventory, and typed posts/query, post CRUD/status, and post metadata/cache Haxe adapter-contract candidates have receipts; broad Core implementation remains open through remaining seeded follow-up domains | Continue WPHX-307 with revisions, WP_Query ABI/query-state, and live DB SQL/result oracle fixtures, then proceed through WPHX-308 through WPHX-320 by source-domain priority |
