...
Source checkpoint: `WPHX-310.06 theme customizer/widget/nav-menu surface inventory`
Tracking issue: `WPHX-000.02`

## Super Progress

Current super progress toward "WordPress completely in Haxe and functional": **43.3%**.
...
| WordPress PHP runtime Haxe ownership | 40 | 30.9 | 12.4 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, bounded WPHX-306 users/auth, WPHX-307 posts/query, WPHX-308 taxonomy/comment, WPHX-309 routing/template domain closures, plus WPHX-310 surface inventory, a first theme/template typed Haxe adapter-contract candidate, theme support/template oracle-source-mirror evidence, bounded WP_Theme_JSON/global styles fixture evidence, and resolver/global-styles wrapper/cache fixture evidence, but much of Core remains unported |
...
| `WPHX-300` | WordPress PHP core | open | 8/8 original direct children closed; WPHX-306, WPHX-307, WPHX-308, and WPHX-309 are closed; WPHX-310 is active; WPHX-312 through WPHX-320 remain seeded follow-up domains | Bootstrap, hooks, error/formatting/escaping/KSES, options/cache, wpdb, REST, multisite, first-party PHP manifest/split closure, users/auth adapter-contract/domain-closure evidence, bounded posts/query domain closure, bounded taxonomy/comment domain closure, bounded rewrite/routing/canonical/template domain closure, themes/theme JSON/template hierarchy surface inventory, first typed theme/template adapter-contract evidence, theme support/template oracle-source-mirror evidence, WP_Theme_JSON/global-styles fixture evidence, resolver/global-styles wrapper/cache fixture evidence, and narrow customizer/widget/nav-menu/admin-theme surface evidence have receipts; broad Core implementation remains open through remaining seeded follow-up domains | Continue WPHX-310 with customizer manager/settings fixtures, widget/sidebar and nav-menu fixtures, installed front-end/admin gates, and upstream ratchets, while generated public PHP adapter replacement stays a later cross-domain gate |
...
| `WPHX-310` Themes/theme JSON/template hierarchy | active | `WPHX-310.01` adds a bounded surface inventory for 127 PHP runtime source files, 152 PHP distribution artifacts, 1,786 ABI entries, and 141 upstream tests. `WPHX-310.02` adds the first typed Haxe theme/template adapter-contract candidate with 40 deterministic observations. `WPHX-310.03` adds a copied-oracle fixture over mirrored `theme.php` and `template.php` with 8 matching oracle/candidate cases for theme support and classic template behavior. `WPHX-310.04` adds a copied-oracle fixture over `WP_Theme_JSON` and schema migration with 5 matching oracle/candidate cases for v2-to-v3 migration, preset normalization, stylesheet variable/preset generation, merge precedence, and insecure style removal. `WPHX-310.05` adds a copied-oracle fixture over `WP_Theme_JSON_Resolver` and global styles/settings wrappers with 6 matching oracle/candidate cases for core/theme merge, block-style variation partials, wrapper caches, global settings/styles/stylesheet output, theme.json support/template-part metadata, theme file URI resolution, and cache cleaning. `WPHX-310.06` narrows the remaining customizer/widget/nav-menu/admin-theme work to 103 source files, 104 distribution artifacts, 1,207 ABI entries, 461 literal hook observations, and 33 upstream tests across customizer, widgets/sidebars, nav menus, and admin theme screens. This remains bounded: it does not claim generated public PHP replacement, user global-styles CPT mutation, customizer transaction parity, widget/nav-menu runtime parity, installed rendering/admin behavior, or upstream parity. Next gates are customizer manager/settings fixtures, widget/sidebar and nav-menu fixtures, installed front-end/admin gates, and selected upstream ratchets. |
