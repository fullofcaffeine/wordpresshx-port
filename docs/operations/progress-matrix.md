...
Source checkpoint: `WPHX-310.02 theme/template adapter-contract candidate`
Tracking issue: `WPHX-000.02`

## Super Progress

Current super progress toward "WordPress completely in Haxe and functional": **43.0%**.
...
| WordPress PHP runtime Haxe ownership | 40 | 30.2 | 12.1 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, bounded WPHX-306 users/auth, WPHX-307 posts/query, WPHX-308 taxonomy/comment, WPHX-309 routing/template domain closures, plus WPHX-310 surface inventory and a first theme/template typed Haxe adapter-contract candidate, but much of Core remains unported |
...
| `WPHX-310` Themes/theme JSON/template hierarchy | active | `WPHX-310.01` adds a bounded surface inventory for 127 PHP runtime source files, 152 PHP distribution artifacts, 1,786 ABI entries, and 141 upstream tests. `WPHX-310.02` adds the first typed Haxe theme/template adapter-contract candidate with 40 deterministic observations across theme support, theme.json/global-styles provenance, template hierarchy, customizer routing, nav menus, widgets/sidebars, and hook intent. This is helper-scope evidence only: it does not claim generated public PHP replacement, installed WordPress behavior, rendered theme parity, customizer transaction parity, or upstream parity. Next gates are classic and block theme fixtures, theme.json/global styles fixtures, theme-support/customizer/widget/nav-menu coverage, installed front-end/admin gates, and selected upstream ratchets. |
