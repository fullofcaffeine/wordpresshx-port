...
Source checkpoint: `WPHX-312.01 HTTP/cron/mail/feed/embed surface inventory`
...
| WordPress PHP runtime Haxe ownership | 40 | 32.6 | 13.0 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, bounded WPHX-306 users/auth, WPHX-307 posts/query, WPHX-308 taxonomy/comment, WPHX-309 routing/template, WPHX-310 theme/theme JSON/template hierarchy domain closures, plus a WPHX-312 HTTP/cron/mail/feed/embed surface inventory, but much of Core remains unported |
...
| `WPHX-312` HTTP/cron/mail/feeds/embeds | active | `WPHX-312.01` adds a bounded surface inventory for 91 PHP runtime source files, 92 PHP distribution artifacts, 708 ABI entries, and 38 upstream tests across HTTP/Requests integration, AI-client HTTP, cron scheduling, PHPMailer/mail, feeds/OPML, embeds/oEmbed, HTTPS detection/migration, privacy request mail tables, and trackbacks. This is inventory-only evidence: it does not claim Haxe runtime ownership, generated public PHP replacement, network side-effect parity, mail transport parity, scheduling parity, feed rendering parity, embed/oEmbed behavior parity, installed behavior, upstream PHPUnit parity, or ecosystem coverage. |
