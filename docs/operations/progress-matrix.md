...
Source checkpoint: `WPHX-310.03 theme support/template oracle fixture`
Tracking issue: `WPHX-000.02`

## Super Progress

Current super progress toward "WordPress completely in Haxe and functional": **43.1%**.
...
| WordPress PHP runtime Haxe ownership | 40 | 30.4 | 12.2 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, bounded WPHX-306 users/auth, WPHX-307 posts/query, WPHX-308 taxonomy/comment, WPHX-309 routing/template domain closures, plus WPHX-310 surface inventory, a first theme/template typed Haxe adapter-contract candidate, and a theme support/template oracle-source-mirror fixture, but much of Core remains unported |
...
| `WPHX-310` Themes/theme JSON/template hierarchy | active | `WPHX-310.01` adds a bounded surface inventory for 127 PHP runtime source files, 152 PHP distribution artifacts, 1,786 ABI entries, and 141 upstream tests. `WPHX-310.02` adds the first typed Haxe theme/template adapter-contract candidate with 40 deterministic observations. `WPHX-310.03` adds a copied-oracle fixture over mirrored `theme.php` and `template.php` with 8 matching oracle/candidate cases for post-thumbnail support merging, html5 defaults, custom-logo defaults, theme feature REST schema registration, block-theme default supports, classic template hierarchy, template loading with args, and theme-compat fallback. This remains bounded: it does not claim generated public PHP replacement, full theme.json merge behavior, customizer transaction parity, installed rendering/admin behavior, or upstream parity. Next gates are theme.json/global-styles fixtures, customizer/widget/nav-menu coverage, installed front-end/admin gates, and selected upstream ratchets. |
