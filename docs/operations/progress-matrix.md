...
Source checkpoint: `WPHX-307.05 post revisions adapter-contract candidate`
...
| WordPress PHP runtime Haxe ownership | 40 | 22 | 8.8 | `WPHX-300` has verified vertical slices, first-party PHP manifest ownership/split closure, user/auth and posts/query Haxe adapter-contract candidates, auth-domain oracle fixtures, and WPHX-307 posts/query, post CRUD/status, post metadata/cache, and post revisions adapter-contract evidence, but much of Core remains unported |
