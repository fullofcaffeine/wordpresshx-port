...
Do not assume stock Haxe PHP can emit the public WordPress distribution files modern Core requires. Use stock Haxe PHP first for private implementation output and Haxe stdlib/runtime behavior; use WPHX PHP for bounded original-path public adapters where file identity, declaration timing, references, native arrays, warnings, stack traces, reflection, or include behavior are the contract. If the stock Haxe PHP target cannot emit an idiomatic private implementation shape, or if WPHX PHP starts becoming a general backend, reduce the problem to a generic compiler-pressure fixture. Consider Reflaxe or a custom/forked PHP target only with evidence and an ADR; do not let convenience PHP strings in runners become the architecture.
