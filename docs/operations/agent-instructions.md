...
Use the shell-retirement ladder from `docs/operations/ownership-state-model.md`: `bridge_shell`, `generated_helper_with_temporary_shell`, `compiler_emitted_original_path_shell`, `durable_public_adapter`, and `whole_file_owned`. Copied, transformed, hand-authored, and JS-patched shells can support bridge or helper-candidate evidence only. Durable public PHP claims must cite compiler-emitted original-path shell evidence, durable adapter evidence, whole-file evidence, or an accepted backend/custom-target improvement.

Do not assume stock Haxe PHP can emit the public WordPress distribution files modern Core requires. ADR-016 makes WPHX PHP compiler usability the near-term focus: route new public WordPress PHP emission gaps to WPHX PHP where file identity, declaration timing, references, native arrays, warnings, stack traces, reflection, or include behavior are the contract. Stock Haxe PHP remains a bounded private-output/stdphp oracle while `WPHX-COMP-PHP-USABLE` proves what the custom compiler can own next; do not shape Haxe source around weak stock-target output. WPHX PHP is the active staged custom compiler lane and uses Reflaxe infrastructure, but it is not yet a mature arbitrary-Haxe PHP target; design Haxe ABI metadata and Adapter IR as future-backend inputs, not as disposable shell hints. If stock Haxe PHP cannot emit an idiomatic private implementation shape, or if WPHX PHP starts becoming a general backend, reduce the problem to a generic compiler-pressure fixture and either add reusable WPHX PHP core nodes, adapt stock std/php behavior, or promote WPHX PHP toward a broader Reflaxe/custom PHP backend with evidence, generated PHP quality gates, and an ADR. Do not let convenience PHP strings in runners become the architecture.
