...
The v0 implementation in `src/wphx/compiler/php/WphxPhpCompiler.hx` covers the currently proven shell shapes: original-path files, guarded global functions, classes/interfaces, bounded direct file-scope script adapters, methods, properties, constants, Haxe runtime bootstrap markers, protected methods, by-reference parameters, and emission-manifest declarations. `WPHX-COMP-PHP-CORE-IR-NATIVE-ARRAYS` and `WPHX-COMP-PHP.07` add the first reusable PHP-core method-body nodes for conditionals, `for`/`foreach` loops, `break`, `continue`, `return`, native array reads/writes/appends, array casts, int/string casts, long array literals, object construction, local variables, assignments, and calls. `WPHX-COMP-PHP-INCLUDE-SIDE-EFFECTS` adds the first bounded original-path direct script adapter for include timing, include returns, function-scope locals, repeated include/include_once behavior, and output buffering. These nodes are reusable compiler core where possible; the WordPress profile only chooses when an original-path public boundary needs them.
