...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_69_bootstrap_haxe' ) ) {
\tfunction wphx_312_69_bootstrap_haxe() {
...
\t\t$wphx_312_69_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_69_lib );
...
function transformCandidateRequestRedirectOptions() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\tif ( empty( $parsed_args['redirection'] ) ) {
\t\t\t$options['follow_redirects'] = false;
\t\t} else {
\t\t\t$options['redirects'] = $parsed_args['redirection'];
\t\t}
`;
  const replacement = `\t\tif ( ${HAXE_MODULE}::shouldDisableRedirects( (int) $parsed_args['redirection'] ) ) {
\t\t\t$options['follow_redirects'] = false;
\t\t} else {
\t\t\t$options['redirects'] = $parsed_args['redirection'];
\t\t}
`;
  ...
  if (transformed === source) throw new Error("Unable to replace WP_Http::request redirect option decision branch");
...
async function main() {
  ...
  transformCandidateRequestRedirectOptions();
  writeProbe();
  ...
  const manifest = {
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request redirect follow-disable branch decision is emitted by generated Haxe PHP. The surrounding public method body, redirect count value, and Requests option array remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
