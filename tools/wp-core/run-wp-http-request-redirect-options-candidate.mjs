...
const ISSUE = {
  id: "wordpresshx-7w1",
  external_ref: "WPHX-312.69",
  title: "WPHX-312.69 - Promote WP_Http request redirect option decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T07:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-redirect-options-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-redirect-options-candidate.hxml";
const WPHX_PHP_HXML = "fixtures/wphx-php/wp-http-request-nonblocking.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-69";
...
const OUT = "manifests/wp-core/wphx-312-69-wp-http-request-redirect-options-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-69-wp-http-request-redirect-options-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-69-wp-http-request-redirect-options-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestRedirectOptionsCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestRedirectOptions\\HttpRequestRedirectOptions_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request zero redirection disables follow_redirects decision"];
const CASES = [
  { id: "wp-http-request:head-defaults", focus: "HEAD request default redirection=0 disables Requests follow_redirects" },
  { id: "wp-http-request:headers-cookies-ssl-post", focus: "nonzero redirection count flows to Requests redirects option" }
];
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-redirect-options-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request method-derived redirect option decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request branch decision that a zero parsed redirection count disables Requests follow_redirects while nonzero counts keep the redirects option path. PHP still owns public WP_Http::request ABI, redirection defaults and filters, HEAD redirection override, hook registration, header/cookie/SSL/max_bytes/data_format handoff, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-redirect-options-candidate",
        "npm run wp:core:wphx-312-wp-http-request-redirect-options-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run wphx:php:adapter-raw-blocks:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-69-wp-http-request-redirect-options-candidate"],
...
async function main() {
  ...
  const wphxPhpShape = {
    ...
    redirect_options_haxe_call: generatedShell.includes(`${HAXE_MODULE}::shouldDisableRedirects`),
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-redirect-options-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request redirect follow-disable branch decision is claimed by this candidate. The surrounding public method body is now emitted by structured WPHX PHP Adapter IR as a bounded original-path adapter and remains PHP-owned request orchestration, not full request behavior ownership.",
      adapter_ir: {
    ...
    fixture: {
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted redirect decision is a pure int-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-redirect-options-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the zero-redirection follow_redirects branch decision is Haxe-owned. Redirection defaults/filters, redirect count values, hook registration, headers, cookies, SSL verification, max_bytes, data_format, Requests dispatch, and response filtering remain PHP-owned in this candidate."
      },
      {
      ...
      },
      {
        ...
        owner: ISSUE.external_ref,
        detail:
          "The candidate consumes a compiler-emitted original-path class-wp-http.php shell for a bounded WP_Http::request redirect-options branch gate plus supporting request/block_request branches. Broader request semantics, whole-file WP_Http ownership, and installed distribution behavior remain later compiler-driven gates."
      }
    ],
    ...
    validation_result: {
      ...
      redirect_following_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-69-wp-http-request-redirect-options-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request redirect options Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request redirect options Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request redirect options candidate" },
