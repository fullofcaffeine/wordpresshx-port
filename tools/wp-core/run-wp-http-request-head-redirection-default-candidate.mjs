...
const ISSUE = {
  id: "wordpresshx-eie",
  external_ref: "WPHX-312.83",
  title: "WPHX-312.83 - Promote WP_Http request HEAD redirection default decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T06:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-head-redirection-default-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-head-redirection-default-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-83";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-83-wp-http-request-head-redirection-default-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-83-wp-http-request-head-redirection-default-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-83-wp-http-request-head-redirection-default-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestHeadRedirectionDefaultCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestHeadRedirectionDefault\\HttpRequestHeadRedirectionDefault_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request explicit HEAD method disables default redirection decision"];
const CASES = [
  { id: "wp-http-request:head-defaults", focus: "explicit HEAD method sets default redirection to 0" },
  { id: "wp-http-request:headers-cookies-ssl-post", focus: "non-HEAD request keeps caller redirection handoff" }
];
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_83_bootstrap_haxe' ) ) {
\tfunction wphx_312_83_bootstrap_haxe() {
...
\t\t$wphx_312_83_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_83_lib );
...
function transformCandidateRequestHeadRedirectionDefault() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\tif ( isset( $args['method'] ) && 'HEAD' === $args['method'] ) {
\t\t\t$defaults['redirection'] = 0;
\t\t}
`;
  const replacement = `\t\tif ( ${HAXE_MODULE}::shouldDisableHeadDefaultRedirection( isset( $args['method'] ), isset( $args['method'] ) ? (string) $args['method'] : '' ) ) {
\t\t\t$defaults['redirection'] = 0;
\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request HEAD redirection default decision branch");
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-head-redirection-default-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request HEAD redirection default decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request pre-parse decision that an explicitly supplied HEAD method sets the default redirection count to 0 before wp_parse_args merges defaults. PHP still owns public WP_Http::request ABI, wp_parse_args behavior, default/filter values, method normalization, parsed args, Requests redirect option handoff, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-head-redirection-default-candidate",
        "npm run wp:core:wphx-312-wp-http-request-head-redirection-default-candidate:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-83-wp-http-request-head-redirection-default-candidate"],
...
async function main() {
  ...
  transformCandidateRequestHeadRedirectionDefault();
  writeProbe();
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-head-redirection-default-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request explicit-HEAD default redirection branch decision is emitted by generated Haxe PHP. The surrounding public method body, wp_parse_args behavior, default filters, and Requests option array remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted HEAD default redirection decision is a pure bool/string-to-bool Haxe function over PHP-owned isset and string coercion and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-head-redirection-default-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the explicit-HEAD default redirection branch decision is Haxe-owned. wp_parse_args behavior, request defaults/filters, method normalization, parsed args, headers, cookies, SSL verification, redirects option handoff, max_bytes, data_format, Requests dispatch, and response filtering remain PHP-owned in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      head_default_redirection_claimed: true,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-83-wp-http-request-head-redirection-default-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request head redirection default Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request head redirection default Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request head redirection default candidate" }
    ],
