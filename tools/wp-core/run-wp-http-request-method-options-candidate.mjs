...
const ISSUE = {
  id: "wordpresshx-4qt",
  external_ref: "WPHX-312.68",
  title: "WPHX-312.68 - Promote WP_Http request body data-format decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T06:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-method-options-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-method-options-candidate.hxml";
const WPHX_PHP_HXML = "fixtures/wphx-php/wp-http-request-nonblocking.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-68";
...
const OUT = "manifests/wp-core/wphx-312-68-wp-http-request-method-options-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-68-wp-http-request-method-options-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-68-wp-http-request-method-options-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestMethodOptionsCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestMethodOptions\\HttpRequestMethodOptions_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request non-GET/HEAD data_format body decision"];
const CASES = [
  { id: "wp-http-request:head-defaults", focus: "HEAD request defaults do not set body data_format" },
  { id: "wp-http-request:headers-cookies-ssl-post", focus: "PUT request body handoff sets Requests data_format=body" }
];
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-method-options-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request method-derived body data-format decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that non-GET/HEAD request methods set Requests data_format='body'. PHP still owns public WP_Http::request ABI, request defaults and filters, HEAD redirection override, hooks, header parsing, cookie normalization, SSL verification, max_bytes handoff, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-method-options-candidate",
        "npm run wp:core:wphx-312-wp-http-request-method-options-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run wphx:php:adapter-raw-blocks:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-68-wp-http-request-method-options-candidate"],
...
async function main() {
  ...
  const wphxPhpShape = {
    ...
    method_options_haxe_call: generatedShell.includes(`${HAXE_MODULE}::shouldUseBodyDataFormat`),
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-method-options-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request method-derived data_format='body' decision is claimed by this candidate. The surrounding public method body is now emitted by structured WPHX PHP Adapter IR as a bounded original-path adapter and remains PHP-owned request orchestration, not full request behavior ownership.",
      adapter_ir: {
    ...
    fixture: {
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted method decision is a pure string-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-method-options-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the non-GET/HEAD data_format decision is Haxe-owned. Method defaults/filters, HEAD redirection override, body serialization, headers, cookies, SSL verification, max_bytes, Requests dispatch, and response filtering remain PHP-owned in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      body_serialization_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-68-wp-http-request-method-options-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request method options Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request method options Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request method options candidate" },
