...
const ISSUE = {
  id: "wordpresshx-l76.9.14",
  external_ref: "WPHX-305.14",
  title: "Promote wpdb result population boundary Haxe candidate slice"
};
const HXML = "fixtures/wp-core/wpdb-result-population-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-305-14";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-305-14-wpdb-result-population-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-305-14-wpdb-result-population-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-305-14-wpdb-result-population-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/WpdbResultPopulationCandidateEntry.hx"
];

const PROMOTED_SYMBOLS = ["maybe_create_table", "maybe_add_column", "dbDelta", "wpdb::query", "wpdb::_do_query", "wpdb::$last_result", "wpdb::$num_rows", "wpdb::get_var", "wpdb::get_row", "wpdb::get_results"];
...
const FIXTURE_CASES = [
  ...
  { id: "runtime:wpdb-result-population-loop", symbol: "wpdb::query/wpdb::$last_result/wpdb::$num_rows", focus: "selected-row last_result population, row-count transitions, empty result clearing, and cached row visibility" },
...
function writeAdminStub(root) {
  ...
  writeFileSync(
    ...
    `<?php
/**
 * WPHX-305.14 live database result population boundary Haxe candidate stub.
 *
...
function haxeBootstrapBlock(dirnameDepth) {
  return `if ( ! function_exists( 'wphx_305_14_bootstrap_haxe' ) ) {
\tfunction wphx_305_14_bootstrap_haxe() {
...
\t\t$wphx_305_14_lib = dirname( __DIR__, ${dirnameDepth} ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_305_14_lib );
...
\t\t} elseif ( \\wphx\\wp\\db\\WpdbQueryState::shouldUseAffectedRows( $wphx_query_kind ) ) {
\t\t\t$this->rows_affected = mysqli_affected_rows( $this->dbh );

\t\t\t// Take note of the insert_id.
\t\t\tif ( \\wphx\\wp\\db\\WpdbQueryState::shouldStoreInsertId( $wphx_query_kind ) ) {
...
\t\t\tif ( \\wphx\\wp\\db\\WpdbResultPopulation::shouldPopulateRows( $this->result instanceof mysqli_result ) ) {
...
\t\tif ( \\wphx\\wp\\db\\WpdbNativeExecution::shouldExecuteNativeQuery( ! empty( $this->dbh ) ) ) {
...
define( 'AUTH_SALT', 'wphx-305-14-auth-salt' );
...
class WPHX_305_14_WPDB extends wpdb {
...
$wpdb = new WPHX_305_14_WPDB( $db_user, $db_password, $db_name, $db_host_with_port );
if ( ! $wpdb->ready ) {
...
function wphx_305_14_fail_on_false( $result, $context ) {
\tglobal $wpdb;
\tif ( false === $result ) {
...
function wphx_305_14_exec( $sql ) {
\tglobal $wpdb;
\treturn wphx_305_14_fail_on_false( $wpdb->query( $sql ), $sql );
}

function wphx_305_14_reset_database() {
\tglobal $wpdb;
...
\twphx_305_14_exec( "SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION'" );
\twphx_305_14_exec( "CREATE TABLE wp_options (
...
\twphx_305_14_exec( "CREATE TABLE wp_existing (id int(11) NOT NULL DEFAULT '0') ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_14_exec( "CREATE TABLE wp_addcol (id int(11) NOT NULL DEFAULT '0') ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_14_exec( "CREATE TABLE wp_delta (
id int(11) NOT NULL DEFAULT '0',
title varchar(20) NOT NULL DEFAULT 'old',
PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_14_exec( "CREATE TABLE wp_users (ID bigint(20) unsigned NOT NULL) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
...
function wphx_305_14_normalize_string( $value ) {
\t$value = (string) $value;
\t$value = preg_replace( '/\\s+/', ' ', $value );
\treturn trim( $value );
}

function wphx_305_14_error_shape( $message ) {
\tif ( '' === (string) $message ) {
\t\treturn array( 'present' => false, 'sha256' => null, 'message' => '' );
\t}
\t$message = wphx_305_14_normalize_string( $message );
\treturn array(
\t\t'present' => true,
...
function wphx_305_14_rows_as_arrays( $rows ) {
\tif ( ! is_array( $rows ) ) {
\t\treturn $rows;
\t}
\t$result = array();
...
function wphx_305_14_query_log_tail( $count ) {
\tglobal $wpdb;
...
function wphx_305_14_table_exists( $table ) {
\tglobal $wpdb;
\treturn $table === $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $wpdb->esc_like( $table ) ) );
}

function wphx_305_14_columns( $table ) {
\t$rows = wphx_305_14_rows( 'DESCRIBE ' . $table );
\t$result = array();
\tforeach ( $rows as $row ) {
\t\t$result[] = array(
\t\t\t'Field'   => $row['Field'],
...
function wphx_305_14_indexes( $table ) {
\t$rows = wphx_305_14_rows( 'SHOW INDEX FROM ' . $table );
\t$result = array();
\tforeach ( $rows as $row ) {
\t\t$result[] = array(
...
function wphx_305_14_option_rows( $names ) {
\tglobal $wpdb;
\tif ( ! $names ) {
\t\treturn array();
\t}
...
function wphx_305_14_cache_pick( $keys ) {
\t$result = array();
\tforeach ( $keys as $key ) {
...
function wphx_305_14_option_snapshot( $keys ) {
\treturn array(
\t\t'rows'  => wphx_305_14_option_rows( $keys ),
\t\t'cache' => array(
...
\t\t\t'keys'           => wphx_305_14_cache_pick( $keys ),
\t\t),
\t);
}

function wphx_305_14_schema_ddl() {
...
function wphx_305_14_case( $id, $symbol, $callback ) {
\twphx_305_14_reset_database();
\t$error = null;
\ttry {
\t\t$value = $callback();
...
function wphx_305_14_run_cases() {
\tglobal $wpdb, $db_name, $db_host_with_port;
\t$cases = array();

\t$cases[] = wphx_305_14_case(
...
\t\t\t\t'existing'      => $existing,
\t\t\t\t'created'       => $created,
\t\t\t\t'existingTable' => wphx_305_14_table_exists( 'wp_existing' ),
\t\t\t\t'createdTable'  => wphx_305_14_table_exists( 'wp_created' ),
\t\t\t\t'createdCols'   => wphx_305_14_columns( 'wp_created' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'existing' => $existing,
\t\t\t\t'added'    => $added,
\t\t\t\t'columns'  => wphx_305_14_columns( 'wp_addcol' ),
\t\t\t);
\t\t}
\t);
...
\t\t'dbDelta',
\t\tfunction () {
\t\t\t$plan = dbDelta( wphx_305_14_schema_ddl(), false );
\t\t\treturn array(
\t\t\t\t'plan'    => $plan,
\t\t\t\t'columns' => wphx_305_14_columns( 'wp_delta' ),
\t\t\t\t'indexes' => wphx_305_14_indexes( 'wp_delta' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'columns' => wphx_305_14_columns( 'wp_delta' ),
\t\t\t\t'indexes' => wphx_305_14_indexes( 'wp_delta' ),
\t\t\t\t'rows'    => wphx_305_14_rows( 'SELECT id, title, slug FROM wp_delta ORDER BY id' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'indexes'              => wphx_305_14_indexes( 'wp_users' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'      => wphx_305_14_option_snapshot( array( 'wphx_added', 'wphx_nonautoload', 'wphx_delete_me' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'        => wphx_305_14_option_snapshot( array( 'wphx_nonautoload', 'wphx_absent_batch', 'wphx_bulk_on', 'wphx_bulk_off' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'deleted'  => $deleted,
\t\t\t\t'after'    => $after,
\t\t\t\t'snapshot' => wphx_305_14_option_snapshot( array( '_transient_wphx_runtime' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'   => wphx_305_14_option_snapshot( array( '_transient_old_value', '_transient_timeout_old_value' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'resultsObject'      => wphx_305_14_rows_as_arrays( $results_object ),
\t\t\t\t'resultsLowerObject' => wphx_305_14_rows_as_arrays( $results_lower_object ),
\t\t\t\t'resultsObjectK'     => wphx_305_14_rows_as_arrays( $results_object_k ),
\t\t\t\t'resultsArrayA'      => $results_array_a,
\t\t\t\t'resultsArrayN'      => $results_array_n,
\t\t\t\t'objectKDuplicate'   => wphx_305_14_rows_as_arrays( $object_k_duplicate ),
\t\t\t\t'colNames'           => $col_names,
...
\t\t\t$failed_insert_error = wphx_305_14_error_shape( $wpdb->last_error );
...
\t\t\t\t'ddlTableExists'            => wphx_305_14_table_exists( 'wp_query_state' ),
\t\t\t\t'selectResult'              => $select_result,
\t\t\t\t'selectNumRows'             => $wpdb->num_rows,
\t\t\t\t'selectRows'                => wphx_305_14_rows_as_arrays( $wpdb->last_result ),
\t\t\t);
\t\t}
\t);
...
\t\t\t$query_log_tail   = wphx_305_14_query_log_tail( 2 );
...
\t$cases[] = wphx_305_14_case(
\t\t'runtime:wpdb-result-population-loop',
...
\t\t\t$multi_last_result = wphx_305_14_rows_as_arrays( $wpdb->last_result );
...
\t\t\t$empty_last_result = wphx_305_14_rows_as_arrays( $wpdb->last_result );
...
\t\t\twphx_305_14_exec( "DROP USER IF EXISTS '$limited_user'@'%'" );
\t\t\twphx_305_14_exec( "CREATE USER '$limited_user'@'%' IDENTIFIED BY '$limited_pass'" );
\t\t\twphx_305_14_exec( "GRANT SELECT ON $db_name.* TO '$limited_user'@'%'" );
\t\t\twphx_305_14_exec( 'FLUSH PRIVILEGES' );
...
\t\t\t\t'createError'       => wphx_305_14_error_shape( $error ),
\t\t\t\t'createdTable'      => wphx_305_14_table_exists( 'wp_permission_denied' ),
\t\t\t);
\t\t}
\t);
...
\t\tfunction () {
\t\t\tglobal $wpdb;
\t\t\twphx_305_14_exec( 'START TRANSACTION' );
\t\t\twphx_305_14_exec( "INSERT INTO wp_options (option_name, option_value, autoload) VALUES ('wphx_txn', 'inside', 'off')" );
\t\t\t$inside = (int) $wpdb->get_var( "SELECT COUNT(*) FROM wp_options WHERE option_name = 'wphx_txn'" );
\t\t\twphx_305_14_exec( 'ROLLBACK' );
\t\t\t$after = (int) $wpdb->get_var( "SELECT COUNT(*) FROM wp_options WHERE option_name = 'wphx_txn'" );
\t\t\twphx_305_14_exec( 'LOCK TABLES wp_options WRITE' );
\t\t\t$open_tables = wphx_305_14_rows( "SHOW OPEN TABLES WHERE In_use > 0" );
\t\t\twphx_305_14_exec( 'UNLOCK TABLES' );
\t\t\treturn array(
...
async function withDbRuntime(runtime, callback) {
  const name = `wordpresshx-wphx-305-14-${runtime.id}-${process.pid}`;
  let containerId = "";
  try {
...
function writeOrCheck(path, contents) {
  if (checkOnly) {
    ...
    if (current !== contents) {
      throw new Error(`${path} is stale; run npm run wp:core:wphx-305-result-population-candidate`);
    }
    return;
  }
...
function ownershipManifest(manifestSha, upstreamDigest, runtimes) {
  return {
    ...
    manifest_id: "ownership:wp-core/wpdb-result-population-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      kind: "module",
      name: "wpdb result population boundary live database Haxe candidate",
      area: "wp-includes/wp-admin",
      public_contract:
        "WordPress-compatible maybe_create_table(), maybe_add_column(), dbDelta(), and wpdb::query/get_var/get_row/get_results remain callable with their public PHP class/function shape while selected schema, dbDelta, and result-population decisions delegate to typed Haxe and continue to execute against locked live MySQL and MariaDB runtimes through wpdb."
    },
    ...
    owned_paths: [...HAXE_SOURCES, "tools/wp-core/run-wpdb-result-population-candidate.mjs", OUT, RECEIPT],
    generated_paths: [OUT_ROOT, OUT, OWNERSHIP, RECEIPT],
    bridge: {
      kind: "generated_shell",
      reason:
        "WordPress core and plugins call global PHP upgrade helpers and wpdb methods with native PHP signatures, mutable public state, and stdClass/array result contracts. The candidate keeps those public PHP shells and delegates normalized table/column existence, dbDelta planning, and bounded result-population decisions to Haxe.",
      bounded_by: [
        "generated candidate wp-admin/includes/upgrade.php shell",
        "generated candidate wp-includes/class-wpdb.php method shell",
        "WPHX-305.14 live MySQL/MariaDB oracle comparison receipt",
    ...
    smell_fixes: [
      {
        description:
          "Separated maybe_create_table()/maybe_add_column() branch decisions, bounded dbDelta classification, wpdb result-shape/null decisions, wpdb::query return/state branch decisions, _do_query timing/logging/execution/reconnect policy, and selected-row last_result/num_rows population policy from native SQL execution into typed Haxe helpers, making the decision surface explicit without changing observable WordPress behavior.",
        compatibility_evidence: ["schema:maybe-create-table", "schema:maybe-add-column", "schema:dbdelta-plan-no-execute", "schema:dbdelta-execute-and-dml", "runtime:wpdb-result-shapes", "runtime:wpdb-native-execution-decisions", "runtime:wpdb-native-execution-log", "runtime:wpdb-result-population-loop"],
        behavior_policy: "no_observable_change"
      }
    ],
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-305-result-population-candidate",
    ...
    notes:
      `The candidate provisions ${runtimes.map((runtime) => runtime.id).join(" and ")} containers from locked index digests and runs the WPHX-305.07 live DB cases plus WPHX-305.11 result-shape, WPHX-305.12 query-state, WPHX-305.13 native-execution, and WPHX-305.14 result-population cases with local PHP mysqli. The broad wp-admin include remains stubbed because this workset is scoped to database helpers, dbDelta planning decisions, wpdb result population, and storage paths.`
  };
}
...
if (!maybeCommand("docker", ["info", "--format", "{{.ServerVersion}}"])) {
  console.error(JSON.stringify({ status: "failed", error: "docker server unavailable; WPHX-305.14 requires live DB containers" }, null, 2));
  process.exit(1);
}
...
const manifest = {
  schema: "wphx.wp-core-wpdb-result-population-candidate.v1",
  issue: ISSUE.external_ref,
  generated_at: RECORDED_AT,
  generator: "tools/wp-core/run-wpdb-result-population-candidate.mjs",
  inputs: {
  ...
  fixture: {
    candidate_kind: "haxe_generated_wpdb_result_population_shell",
    ...
    live_boundaries_covered: [
      ...
      "wpdb::query selected-row last_result population and num_rows behavior over live rows",
    ...
    closes_gaps_from: [
      {
        ...
        resolution:
          "WPHX-305.14 extends the typed Haxe schema-helper/dbDelta candidate with bounded wpdb result-population decisions while retaining live MySQL/MariaDB execution through wpdb."
      },
      {
        manifest: LIVE_DB_CANDIDATE,
        gap: "wpdb-class-not-yet-haxe-owned",
        resolution:
          "WPHX-305.14 resolves a bounded result-population portion of this gap with typed Haxe helpers. Full wpdb query execution, mutation state, replacement/drop-in behavior, and storage ownership remain split to later WPHX-305 slices."
      },
      {
        manifest: DBDELTA_CANDIDATE,
        gap: "wpdb-class-not-yet-haxe-owned",
        resolution:
          "WPHX-305.14 keeps the dbDelta planning helper in the candidate while extending the wpdb class shell."
      },
      {
        manifest: QUERY_RESULT_CANDIDATE,
        gap: "wpdb-query-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.14 keeps the result-shape candidate in the live shell while extending the wpdb result-population boundary."
      },
      {
        manifest: QUERY_STATE_CANDIDATE,
        gap: "wpdb-native-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.14 keeps the query-state candidate in the live shell while extending the wpdb selected-row population boundary."
      },
      {
        manifest: NATIVE_EXECUTION_CANDIDATE,
        gap: "wpdb-native-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.14 follows the native-execution candidate by delegating selected-row population guard, row-count initialization, row-count increment, and selected-row return value to typed Haxe."
      }
    ],
    follows: ["WPHX-305.13", "WPHX-305.12", "WPHX-305.11", "WPHX-305.10", "WPHX-305.08", "WPHX-305.07", "WPHX-305.06"]
  },
  toolchain: {
  ...
  remaining_gaps: [
    {
      id: "wpdb-raw-mysqli-resource-not-yet-haxe-owned",
      owner: "WPHX-305.15",
      detail: "This candidate promotes wpdb::query branch/state decisions, _do_query logging/execution guard/count/reconnect policy, selected-row population policy, and get_var/get_row/get_results return-shape decisions. The raw mysqli_query call, mysqli_result resource/object handling, mysqli_fetch_object ownership, native row-object assignment seam, full dbDelta parser/planner, replacement/drop-in behavior, and option/transient storage remain oracle/native pending later WPHX-305 slices."
    },
    {
...
const receipt = {
  ...
  id: "receipt:wphx-305-14-wpdb-result-population-candidate",
  issue: ISSUE,
  recorded_at: RECORDED_AT,
  artifacts: [
    {
      path: OUT,
      role: "wpdb result population boundary live MySQL/MariaDB Haxe candidate manifest"
    },
    {
      path: OWNERSHIP,
    ...
    {
      path: "tools/wp-core/run-wpdb-result-population-candidate.mjs",
      role: "wpdb result population boundary live DB candidate generator and check-mode validator"
    },
    {
