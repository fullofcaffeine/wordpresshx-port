...
const ISSUE = {
  id: "wordpresshx-l76.9.8",
  external_ref: "WPHX-305.08",
  title: "Add typed Haxe wpdb/schema/option live-DB candidate"
};
const HXML = "fixtures/wp-core/wpdb-live-db-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-305-08";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-305-08-wpdb-live-db-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-305-08-wpdb-live-db-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-305-08-wpdb-live-db-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/WpdbLiveDbCandidateEntry.hx"
];

const PROMOTED_SYMBOLS = ["maybe_create_table", "maybe_add_column"];
...
function writeAdminStub(root) {
  ...
  writeFileSync(
    ...
    `<?php
/**
 * WPHX-305.08 live database Haxe candidate stub.
 *
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_305_08_bootstrap_haxe' ) ) {
\tfunction wphx_305_08_bootstrap_haxe() {
...
\t\t$wphx_305_08_lib = dirname( __DIR__, 3 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_305_08_lib );
...
define( 'AUTH_SALT', 'wphx-305-08-auth-salt' );
...
class WPHX_305_08_WPDB extends wpdb {
...
$wpdb = new WPHX_305_08_WPDB( $db_user, $db_password, $db_name, $db_host_with_port );
if ( ! $wpdb->ready ) {
...
function wphx_305_08_fail_on_false( $result, $context ) {
\tglobal $wpdb;
\tif ( false === $result ) {
...
function wphx_305_08_exec( $sql ) {
\tglobal $wpdb;
\treturn wphx_305_08_fail_on_false( $wpdb->query( $sql ), $sql );
}

function wphx_305_08_reset_database() {
\tglobal $wpdb;
...
\twphx_305_08_exec( "SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION'" );
\twphx_305_08_exec( "CREATE TABLE wp_options (
...
\twphx_305_08_exec( "CREATE TABLE wp_existing (id int(11) NOT NULL DEFAULT '0') ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_08_exec( "CREATE TABLE wp_addcol (id int(11) NOT NULL DEFAULT '0') ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_08_exec( "CREATE TABLE wp_delta (
id int(11) NOT NULL DEFAULT '0',
title varchar(20) NOT NULL DEFAULT 'old',
PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_08_exec( "CREATE TABLE wp_users (ID bigint(20) unsigned NOT NULL) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
...
function wphx_305_08_normalize_string( $value ) {
\t$value = (string) $value;
\t$value = preg_replace( '/\\s+/', ' ', $value );
\treturn trim( $value );
}

function wphx_305_08_error_shape( $message ) {
\tif ( '' === (string) $message ) {
\t\treturn array( 'present' => false, 'sha256' => null, 'message' => '' );
\t}
\t$message = wphx_305_08_normalize_string( $message );
\treturn array(
\t\t'present' => true,
...
function wphx_305_08_table_exists( $table ) {
\tglobal $wpdb;
\treturn $table === $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $wpdb->esc_like( $table ) ) );
}

function wphx_305_08_columns( $table ) {
\t$rows = wphx_305_08_rows( 'DESCRIBE ' . $table );
\t$result = array();
\tforeach ( $rows as $row ) {
\t\t$result[] = array(
\t\t\t'Field'   => $row['Field'],
...
function wphx_305_08_indexes( $table ) {
\t$rows = wphx_305_08_rows( 'SHOW INDEX FROM ' . $table );
\t$result = array();
\tforeach ( $rows as $row ) {
\t\t$result[] = array(
...
function wphx_305_08_option_rows( $names ) {
\tglobal $wpdb;
\tif ( ! $names ) {
\t\treturn array();
\t}
...
function wphx_305_08_cache_pick( $keys ) {
\t$result = array();
\tforeach ( $keys as $key ) {
...
function wphx_305_08_option_snapshot( $keys ) {
\treturn array(
\t\t'rows'  => wphx_305_08_option_rows( $keys ),
\t\t'cache' => array(
...
\t\t\t'keys'           => wphx_305_08_cache_pick( $keys ),
\t\t),
\t);
}

function wphx_305_08_schema_ddl() {
...
function wphx_305_08_case( $id, $symbol, $callback ) {
\twphx_305_08_reset_database();
\t$error = null;
\ttry {
\t\t$value = $callback();
...
function wphx_305_08_run_cases() {
\tglobal $wpdb, $db_name, $db_host_with_port;
\t$cases = array();

\t$cases[] = wphx_305_08_case(
...
\t\t\t\t'existing'      => $existing,
\t\t\t\t'created'       => $created,
\t\t\t\t'existingTable' => wphx_305_08_table_exists( 'wp_existing' ),
\t\t\t\t'createdTable'  => wphx_305_08_table_exists( 'wp_created' ),
\t\t\t\t'createdCols'   => wphx_305_08_columns( 'wp_created' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'existing' => $existing,
\t\t\t\t'added'    => $added,
\t\t\t\t'columns'  => wphx_305_08_columns( 'wp_addcol' ),
\t\t\t);
\t\t}
\t);
...
\t\t'dbDelta',
\t\tfunction () {
\t\t\t$plan = dbDelta( wphx_305_08_schema_ddl(), false );
\t\t\treturn array(
\t\t\t\t'plan'    => $plan,
\t\t\t\t'columns' => wphx_305_08_columns( 'wp_delta' ),
\t\t\t\t'indexes' => wphx_305_08_indexes( 'wp_delta' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'columns' => wphx_305_08_columns( 'wp_delta' ),
\t\t\t\t'indexes' => wphx_305_08_indexes( 'wp_delta' ),
\t\t\t\t'rows'    => wphx_305_08_rows( 'SELECT id, title, slug FROM wp_delta ORDER BY id' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'indexes'              => wphx_305_08_indexes( 'wp_users' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'      => wphx_305_08_option_snapshot( array( 'wphx_added', 'wphx_nonautoload', 'wphx_delete_me' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'        => wphx_305_08_option_snapshot( array( 'wphx_nonautoload', 'wphx_absent_batch', 'wphx_bulk_on', 'wphx_bulk_off' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'deleted'  => $deleted,
\t\t\t\t'after'    => $after,
\t\t\t\t'snapshot' => wphx_305_08_option_snapshot( array( '_transient_wphx_runtime' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'   => wphx_305_08_option_snapshot( array( '_transient_old_value', '_transient_timeout_old_value' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\twphx_305_08_exec( "DROP USER IF EXISTS '$limited_user'@'%'" );
\t\t\twphx_305_08_exec( "CREATE USER '$limited_user'@'%' IDENTIFIED BY '$limited_pass'" );
\t\t\twphx_305_08_exec( "GRANT SELECT ON $db_name.* TO '$limited_user'@'%'" );
\t\t\twphx_305_08_exec( 'FLUSH PRIVILEGES' );
...
\t\t\t\t'createError'       => wphx_305_08_error_shape( $error ),
\t\t\t\t'createdTable'      => wphx_305_08_table_exists( 'wp_permission_denied' ),
\t\t\t);
\t\t}
\t);
...
\t\tfunction () {
\t\t\tglobal $wpdb;
\t\t\twphx_305_08_exec( 'START TRANSACTION' );
\t\t\twphx_305_08_exec( "INSERT INTO wp_options (option_name, option_value, autoload) VALUES ('wphx_txn', 'inside', 'off')" );
\t\t\t$inside = (int) $wpdb->get_var( "SELECT COUNT(*) FROM wp_options WHERE option_name = 'wphx_txn'" );
\t\t\twphx_305_08_exec( 'ROLLBACK' );
\t\t\t$after = (int) $wpdb->get_var( "SELECT COUNT(*) FROM wp_options WHERE option_name = 'wphx_txn'" );
\t\t\twphx_305_08_exec( 'LOCK TABLES wp_options WRITE' );
\t\t\t$open_tables = wphx_305_08_rows( "SHOW OPEN TABLES WHERE In_use > 0" );
\t\t\twphx_305_08_exec( 'UNLOCK TABLES' );
\t\t\treturn array(
...
async function withDbRuntime(runtime, callback) {
  const name = `wordpresshx-wphx-305-08-${runtime.id}-${process.pid}`;
  let containerId = "";
  try {
...
function writeOrCheck(path, contents) {
  if (checkOnly) {
    ...
    if (current !== contents) {
      throw new Error(`${path} is stale; run npm run wp:core:wphx-305-live-db-candidate`);
    }
    return;
  }
...
function ownershipManifest(manifestSha, upstreamDigest, runtimes) {
  return {
    ...
    manifest_id: "ownership:wp-core/wpdb-live-db-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      kind: "module",
      name: "wpdb live database schema helper Haxe candidate",
      area: "wp-includes/wp-admin",
      public_contract:
        "WordPress-compatible maybe_create_table() and maybe_add_column() global PHP functions remain callable from wp-admin/includes/upgrade.php while selected schema-helper branch decisions delegate to typed Haxe and continue to execute against locked live MySQL and MariaDB runtimes through wpdb."
    },
    ...
    owned_paths: [...HAXE_SOURCES, "tools/wp-core/run-wpdb-live-db-candidate.mjs", OUT, RECEIPT],
    generated_paths: [OUT_ROOT, OUT, OWNERSHIP, RECEIPT],
    bridge: {
      kind: "generated_shell",
      reason:
        "WordPress core and plugins call global PHP upgrade helpers with native wpdb state. The candidate keeps the public PHP function shell and delegates only normalized table/column existence decisions to Haxe.",
      bounded_by: [
        "generated candidate wp-admin/includes/upgrade.php shell",
        "WPHX-305.08 live MySQL/MariaDB oracle comparison receipt",
    ...
    smell_fixes: [
      {
        description:
          "Separated maybe_create_table()/maybe_add_column() branch decisions from native SQL execution into typed Haxe helpers, making the decision surface explicit without changing observable WordPress behavior.",
        compatibility_evidence: ["schema:maybe-create-table", "schema:maybe-add-column"],
        behavior_policy: "no_observable_change"
      }
    ],
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-305-live-db-candidate",
      ...
      receipt_refs: ["receipt:wphx-305-08-wpdb-live-db-candidate", "receipt:wphx-305-07-wpdb-live-db-runtime-gate"],
      manifest_digest: manifestSha
    },
    notes:
      `The candidate provisions ${runtimes.map((runtime) => runtime.id).join(" and ")} containers from locked index digests and runs the WPHX-305.07 live DB cases with local PHP mysqli. The broad wp-admin include remains stubbed because this workset is scoped to database helpers and storage paths.`
  };
}
...
if (!maybeCommand("docker", ["info", "--format", "{{.ServerVersion}}"])) {
  console.error(JSON.stringify({ status: "failed", error: "docker server unavailable; WPHX-305.08 requires live DB containers" }, null, 2));
  process.exit(1);
}
...
const manifest = {
  schema: "wphx.wp-core-wpdb-live-db-candidate.v1",
  issue: ISSUE.external_ref,
  generated_at: RECORDED_AT,
  generator: "tools/wp-core/run-wpdb-live-db-candidate.mjs",
  inputs: {
  ...
  fixture: {
    candidate_kind: "haxe_generated_schema_helper_shell",
    ...
    closes_gaps_from: [
      {
        ...
        resolution:
          "WPHX-305.08 installs the first typed Haxe schema-helper candidate for maybe_create_table() and maybe_add_column() while retaining live MySQL/MariaDB execution through wpdb."
      }
    ],
    follows: ["WPHX-305.07", "WPHX-305.06"]
  },
  toolchain: {
  ...
  remaining_gaps: [
    {
      ...
      owner: "WPHX-305.10",
      detail: "This first candidate promotes only maybe_create_table()/maybe_add_column() branch decisions. The wpdb class, query/result handling, dbDelta parser/planner, and option/transient storage remain oracle/native pending later WPHX-305 slices."
    },
    {
...
const receipt = {
  ...
  id: "receipt:wphx-305-08-wpdb-live-db-candidate",
  issue: ISSUE,
  recorded_at: RECORDED_AT,
  artifacts: [
    {
      path: OUT,
      role: "wpdb live MySQL/MariaDB Haxe candidate manifest"
    },
    {
      path: OWNERSHIP,
    ...
    {
      path: "tools/wp-core/run-wpdb-live-db-candidate.mjs",
      role: "live DB candidate generator and check-mode validator"
    },
    {
