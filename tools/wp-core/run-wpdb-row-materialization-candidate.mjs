...
const ISSUE = {
  id: "wordpresshx-l76.9.21",
  external_ref: "WPHX-305.21",
  title: "Type wpdb fetched row object materialization"
};
const HXML = "fixtures/wp-core/wpdb-row-materialization-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-305-21";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const MYSQLI_EXECUTION_PHP = `${HAXE_OUT}/lib/wphx/wp/db/WpdbMysqliExecution.php`;
const ROW_MATERIALIZATION_PHP = `${HAXE_OUT}/lib/wphx/wp/db/WpdbRowMaterialization.php`;
const OUT = "manifests/wp-core/wphx-305-21-wpdb-row-materialization-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-305-21-wpdb-row-materialization-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-305-21-wpdb-row-materialization-candidate.v1.json";
...
const MYSQLI_ROW_TRAVERSAL_CANDIDATE = "manifests/wp-core/wphx-305-17-wpdb-mysqli-row-traversal-candidate.v1.json";
const MYSQLI_GLOBAL_LOWERING_PROOF = "manifests/wp-core/wphx-305-19-wpdb-mysqli-global-lowering-proof.v1.json";
...
const HAXE_SOURCES = [
  ...
  "src/wphx/wp/db/WpdbMysqliCallGate.hx",
  "src/wphx/wp/db/WpdbMysqliExecution.hx",
  "src/wphx/wp/db/WpdbRowTraversal.hx",
  "src/wphx/wp/db/WpdbRowMaterialization.hx",
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/WpdbRowMaterializationCandidateEntry.hx"
];
...
const PROMOTED_DECISIONS = [
  ...
  "wpdb::query fetched-row presence decision",
  "wpdb::query last_result assignment slot decision",
  "wpdb::query fetched-row count advance decision",
  ...
  "wpdb::_do_query native mysqli query call gate decision",
  "wpdb::query native mysqli fetch-object call gate decision",
  "wpdb::_do_query typed @:phpGlobal mysqli_query execution",
  "wpdb::query typed @:phpGlobal mysqli_fetch_object execution",
  "wpdb::query fetched-row object presence materialization",
  "wpdb::get_var row field value extraction",
  "wpdb::get_row row fields ARRAY_A materialization",
  "wpdb::get_row row values ARRAY_N materialization",
  "wpdb::get_results OBJECT_K first-field key materialization",
  "wpdb::get_results ARRAY_A/ARRAY_N row materialization",
...
const FIXTURE_CASES = [
  ...
  { id: "runtime:wpdb-mysqli-row-traversal-loop", symbol: "wpdb::query/wpdb::$last_result/wpdb::$num_rows", focus: "selected-row last_result population, row-presence break policy, assignment slot policy, row-count transitions, empty result clearing, and cached row visibility" },
  { id: "runtime:wpdb-mysqli-row-traversal-metadata", symbol: "wpdb::query/wpdb::$dbh/wpdb::$result/wpdb::$rows_affected/wpdb::$insert_id", focus: "mysqli handle/result recognition, affected-row/insert-id metadata reads, and empty-handle reconnect fallback" },
...
function writeAdminStub(root) {
  ...
  writeFileSync(
    ...
    `<?php
/**
 * WPHX-305.21 live database mysqli row traversal Haxe candidate stub.
 *
...
function haxeBootstrapBlock(dirnameDepth) {
  return `if ( ! function_exists( 'wphx_305_20_bootstrap_haxe' ) ) {
\tfunction wphx_305_20_bootstrap_haxe() {
...
\t\t$wphx_305_20_lib = dirname( __DIR__, ${dirnameDepth} ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_305_20_lib );
...
function transformCandidateWpdb() {
  ...
  let source = installBootstrap(readFileSync(path, "utf8"), 2);
  source = replaceFunction(
    source,
    "query",
...
\t\t\tif ( \\wphx\\wp\\db\\WpdbMysqliCallGate::shouldFetchRows( $wphx_should_fetch_rows ) ) {
\t\t\t\twhile ( true ) {
\t\t\t\t\t$row = \\wphx\\wp\\db\\WpdbMysqliExecution::fetchObject( $this->result );

\t\t\t\t\tif ( ! \\wphx\\wp\\db\\WpdbRowMaterialization::hasFetchedRow( $row ) ) {
\t\t\t\t\t\tbreak;
\t\t\t\t\t}

\t\t\t\t\t$this->last_result[ \\wphx\\wp\\db\\WpdbRowTraversal::assignmentIndex( $num_rows ) ] = $row;
\t\t\t\t\t$num_rows = \\wphx\\wp\\db\\WpdbRowTraversal::nextFetchedRowCount( $num_rows );
\t\t\t\t}
\t\t\t}
...
\t\tif ( \\wphx\\wp\\db\\WpdbMysqliCallGate::shouldCallQuery( $wphx_should_execute_native_query ) ) {
\t\t\t$this->result = \\wphx\\wp\\db\\WpdbMysqliExecution::query( $this->dbh, $query );
\t\t}
...
\t\t\t$values = \\wphx\\wp\\db\\WpdbRowMaterialization::rowValues( $this->last_result[ $y ] );
\t\t}
...
\t\t\treturn \\wphx\\wp\\db\\WpdbRowMaterialization::rowFields( $this->last_result[ $y ] );
\t\t}

\t\tif ( 'array_n' === $wphx_output_kind ) {
\t\t\treturn \\wphx\\wp\\db\\WpdbRowMaterialization::rowValues( $this->last_result[ $y ] );
\t\t}
...
\t\t\t\t\t$key = \\wphx\\wp\\db\\WpdbRowMaterialization::firstFieldValue( $row );
...
\t\t\t\t\tif ( 'array_a' === $wphx_output_kind ) {
\t\t\t\t\t\t$new_array[] = \\wphx\\wp\\db\\WpdbRowMaterialization::rowFields( $row );
\t\t\t\t\t} else {
\t\t\t\t\t\t$new_array[] = \\wphx\\wp\\db\\WpdbRowMaterialization::rowValues( $row );
\t\t\t\t\t}
\t\t\t\t}
\t\t\t\treturn $new_array;
\t\t\t}
...
define( 'AUTH_SALT', 'wphx-305-21-auth-salt' );
...
class WPHX_305_20_WPDB extends wpdb {
...
$wpdb = new WPHX_305_20_WPDB( $db_user, $db_password, $db_name, $db_host_with_port );
if ( ! $wpdb->ready ) {
...
function wphx_305_20_fail_on_false( $result, $context ) {
\tglobal $wpdb;
\tif ( false === $result ) {
...
function wphx_305_20_exec( $sql ) {
\tglobal $wpdb;
\treturn wphx_305_20_fail_on_false( $wpdb->query( $sql ), $sql );
}

function wphx_305_20_reset_database() {
\tglobal $wpdb;
...
\twphx_305_20_exec( "SET SESSION sql_mode = 'NO_ENGINE_SUBSTITUTION'" );
\twphx_305_20_exec( "CREATE TABLE wp_options (
...
\twphx_305_20_exec( "CREATE TABLE wp_existing (id int(11) NOT NULL DEFAULT '0') ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_20_exec( "CREATE TABLE wp_addcol (id int(11) NOT NULL DEFAULT '0') ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_20_exec( "CREATE TABLE wp_delta (
id int(11) NOT NULL DEFAULT '0',
title varchar(20) NOT NULL DEFAULT 'old',
PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
\twphx_305_20_exec( "CREATE TABLE wp_users (ID bigint(20) unsigned NOT NULL) ENGINE=InnoDB DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci" );
...
function wphx_305_20_normalize_string( $value ) {
\t$value = (string) $value;
\t$value = preg_replace( '/\\s+/', ' ', $value );
\treturn trim( $value );
}

function wphx_305_20_error_shape( $message ) {
\tif ( '' === (string) $message ) {
\t\treturn array( 'present' => false, 'sha256' => null, 'message' => '' );
\t}
\t$message = wphx_305_20_normalize_string( $message );
\treturn array(
\t\t'present' => true,
...
function wphx_305_20_rows_as_arrays( $rows ) {
\tif ( ! is_array( $rows ) ) {
\t\treturn $rows;
\t}
\t$result = array();
...
function wphx_305_20_query_log_tail( $count ) {
\tglobal $wpdb;
...
function wphx_305_20_table_exists( $table ) {
\tglobal $wpdb;
\treturn $table === $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $wpdb->esc_like( $table ) ) );
}

function wphx_305_20_columns( $table ) {
\t$rows = wphx_305_20_rows( 'DESCRIBE ' . $table );
\t$result = array();
\tforeach ( $rows as $row ) {
\t\t$result[] = array(
\t\t\t'Field'   => $row['Field'],
...
function wphx_305_20_indexes( $table ) {
\t$rows = wphx_305_20_rows( 'SHOW INDEX FROM ' . $table );
\t$result = array();
\tforeach ( $rows as $row ) {
\t\t$result[] = array(
...
function wphx_305_20_option_rows( $names ) {
\tglobal $wpdb;
\tif ( ! $names ) {
\t\treturn array();
\t}
...
function wphx_305_20_cache_pick( $keys ) {
\t$result = array();
\tforeach ( $keys as $key ) {
...
function wphx_305_20_option_snapshot( $keys ) {
\treturn array(
\t\t'rows'  => wphx_305_20_option_rows( $keys ),
\t\t'cache' => array(
...
\t\t\t'keys'           => wphx_305_20_cache_pick( $keys ),
\t\t),
\t);
}

function wphx_305_20_schema_ddl() {
...
function wphx_305_20_case( $id, $symbol, $callback ) {
\twphx_305_20_reset_database();
\t$error = null;
\ttry {
\t\t$value = $callback();
...
function wphx_305_20_run_cases() {
\tglobal $wpdb, $db_name, $db_host_with_port;
\t$cases = array();

\t$cases[] = wphx_305_20_case(
...
\t\t\t\t'existing'      => $existing,
\t\t\t\t'created'       => $created,
\t\t\t\t'existingTable' => wphx_305_20_table_exists( 'wp_existing' ),
\t\t\t\t'createdTable'  => wphx_305_20_table_exists( 'wp_created' ),
\t\t\t\t'createdCols'   => wphx_305_20_columns( 'wp_created' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'existing' => $existing,
\t\t\t\t'added'    => $added,
\t\t\t\t'columns'  => wphx_305_20_columns( 'wp_addcol' ),
\t\t\t);
\t\t}
\t);
...
\t\t'dbDelta',
\t\tfunction () {
\t\t\t$plan = dbDelta( wphx_305_20_schema_ddl(), false );
\t\t\treturn array(
\t\t\t\t'plan'    => $plan,
\t\t\t\t'columns' => wphx_305_20_columns( 'wp_delta' ),
\t\t\t\t'indexes' => wphx_305_20_indexes( 'wp_delta' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'columns' => wphx_305_20_columns( 'wp_delta' ),
\t\t\t\t'indexes' => wphx_305_20_indexes( 'wp_delta' ),
\t\t\t\t'rows'    => wphx_305_20_rows( 'SELECT id, title, slug FROM wp_delta ORDER BY id' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'indexes'              => wphx_305_20_indexes( 'wp_users' ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'      => wphx_305_20_option_snapshot( array( 'wphx_added', 'wphx_nonautoload', 'wphx_delete_me' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'        => wphx_305_20_option_snapshot( array( 'wphx_nonautoload', 'wphx_absent_batch', 'wphx_bulk_on', 'wphx_bulk_off' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'deleted'  => $deleted,
\t\t\t\t'after'    => $after,
\t\t\t\t'snapshot' => wphx_305_20_option_snapshot( array( '_transient_wphx_runtime' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'snapshot'   => wphx_305_20_option_snapshot( array( '_transient_old_value', '_transient_timeout_old_value' ) ),
\t\t\t);
\t\t}
\t);
...
\t\t\t\t'resultsObject'      => wphx_305_20_rows_as_arrays( $results_object ),
\t\t\t\t'resultsLowerObject' => wphx_305_20_rows_as_arrays( $results_lower_object ),
\t\t\t\t'resultsObjectK'     => wphx_305_20_rows_as_arrays( $results_object_k ),
\t\t\t\t'resultsArrayA'      => $results_array_a,
\t\t\t\t'resultsArrayN'      => $results_array_n,
\t\t\t\t'objectKDuplicate'   => wphx_305_20_rows_as_arrays( $object_k_duplicate ),
\t\t\t\t'colNames'           => $col_names,
...
\t\t\t$failed_insert_error = wphx_305_20_error_shape( $wpdb->last_error );
...
\t\t\t\t'ddlTableExists'            => wphx_305_20_table_exists( 'wp_query_state' ),
\t\t\t\t'selectResult'              => $select_result,
\t\t\t\t'selectNumRows'             => $wpdb->num_rows,
\t\t\t\t'selectRows'                => wphx_305_20_rows_as_arrays( $wpdb->last_result ),
\t\t\t);
\t\t}
\t);
...
\t\t\t$query_log_tail   = wphx_305_20_query_log_tail( 2 );
...
\t$cases[] = wphx_305_20_case(
\t\t'runtime:wpdb-mysqli-row-traversal-loop',
...
\t\t\t$multi_last_result = wphx_305_20_rows_as_arrays( $wpdb->last_result );
...
\t\t\t$empty_last_result = wphx_305_20_rows_as_arrays( $wpdb->last_result );
...
\t$cases[] = wphx_305_20_case(
\t\t'runtime:wpdb-mysqli-row-traversal-metadata',
...
\t\t\t$select_rows = wphx_305_20_rows_as_arrays( $wpdb->last_result );
...
\t\t\t$reconnect_last_error = wphx_305_20_error_shape( $wpdb->last_error );
\t\t\t$reconnect_rows = wphx_305_20_rows_as_arrays( $wpdb->last_result );
...
\t\t\twphx_305_20_exec( "DROP USER IF EXISTS '$limited_user'@'%'" );
\t\t\twphx_305_20_exec( "CREATE USER '$limited_user'@'%' IDENTIFIED BY '$limited_pass'" );
\t\t\twphx_305_20_exec( "GRANT SELECT ON $db_name.* TO '$limited_user'@'%'" );
\t\t\twphx_305_20_exec( 'FLUSH PRIVILEGES' );
...
\t\t\t\t'createError'       => wphx_305_20_error_shape( $error ),
\t\t\t\t'createdTable'      => wphx_305_20_table_exists( 'wp_permission_denied' ),
\t\t\t);
\t\t}
\t);
...
\t\tfunction () {
\t\t\tglobal $wpdb;
\t\t\twphx_305_20_exec( 'START TRANSACTION' );
\t\t\twphx_305_20_exec( "INSERT INTO wp_options (option_name, option_value, autoload) VALUES ('wphx_txn', 'inside', 'off')" );
\t\t\t$inside = (int) $wpdb->get_var( "SELECT COUNT(*) FROM wp_options WHERE option_name = 'wphx_txn'" );
\t\t\twphx_305_20_exec( 'ROLLBACK' );
\t\t\t$after = (int) $wpdb->get_var( "SELECT COUNT(*) FROM wp_options WHERE option_name = 'wphx_txn'" );
\t\t\twphx_305_20_exec( 'LOCK TABLES wp_options WRITE' );
\t\t\t$open_tables = wphx_305_20_rows( "SHOW OPEN TABLES WHERE In_use > 0" );
\t\t\twphx_305_20_exec( 'UNLOCK TABLES' );
\t\t\treturn array(
...
function candidateMysqliIntegration() {
  const shellPath = `${CANDIDATE_ROOT}/wp-includes/class-wpdb.php`;
  const shell = readFileSync(shellPath, "utf8");
  const execution = readFileSync(MYSQLI_EXECUTION_PHP, "utf8");
  const directQuery = /return\s+\\mysqli_query\(\$handle,\s*\$query\);/.test(execution);
  const directFetch = /return\s+\\mysqli_fetch_object\(\$result\);/.test(execution);
  const classStaticMysqliCall = /::\\?mysqli_(query|fetch_object)\s*\(/.test(execution);
  const generatedBoundaryDetected = /WPHX_305_(17|20)_MysqliBoundary/.test(shell);
  const shellUsesQueryHelper = /\\wphx\\wp\\db\\WpdbMysqliExecution::query\(\s*\$this->dbh,\s*\$query\s*\)/.test(shell);
  const shellUsesFetchHelper = /\\wphx\\wp\\db\\WpdbMysqliExecution::fetchObject\(\s*\$this->result\s*\)/.test(shell);
  const shellUsesRawBoundaryCall = /WPHX_305_(17|20)_MysqliBoundary::(native_query|fetch_object)\s*\(/.test(shell);
  const passed = directQuery && directFetch && !classStaticMysqliCall && !generatedBoundaryDetected && shellUsesQueryHelper && shellUsesFetchHelper && !shellUsesRawBoundaryCall;
  return {
    id: "wphx-305-21-live-wpdb-phpglobal-mysqli-integration",
    generated_haxe_execution: {
      path: MYSQLI_EXECUTION_PHP,
      bytes: statSync(MYSQLI_EXECUTION_PHP).size,
      sha256: sha256File(MYSQLI_EXECUTION_PHP),
      evidence_lines: sourceLinesWithNeedles(execution, ["mysqli_query", "mysqli_fetch_object"]),
      direct_query_call_detected: directQuery,
      direct_fetch_object_call_detected: directFetch,
      class_static_mysqli_call_detected: classStaticMysqliCall
    },
    transformed_wpdb_shell: {
      path: shellPath,
      bytes: statSync(shellPath).size,
      sha256: sha256File(shellPath),
      evidence_lines: sourceLinesWithNeedles(shell, ["WpdbMysqliExecution", "WpdbMysqliCallGate", "WPHX_305_17_MysqliBoundary", "WPHX_305_20_MysqliBoundary"]),
      shell_uses_query_helper: shellUsesQueryHelper,
      shell_uses_fetch_helper: shellUsesFetchHelper,
      generated_boundary_detected: generatedBoundaryDetected,
      raw_boundary_call_detected: shellUsesRawBoundaryCall
    },
    selected_strategy: "typed-haxe-phpglobal-mysqli-execution",
    passed
  };
}

function candidateRowMaterialization() {
  const shellPath = `${CANDIDATE_ROOT}/wp-includes/class-wpdb.php`;
  const shell = readFileSync(shellPath, "utf8");
  const materialization = readFileSync(ROW_MATERIALIZATION_PHP, "utf8");
  const directIsObject = /return\s+\\is_object\(\$row\);/.test(materialization);
  const directGetObjectVars = /return\s+\\get_object_vars\(\$row\);/.test(materialization);
  const directArrayValues = /return\s+\\array_values\(WpdbRowMaterialization::rowFields\(\$row\)\);/.test(materialization);
  const directArrayShift = /return\s+\\array_shift\(\$fields\);/.test(materialization);
  const shellUsesPresenceHelper = /\\wphx\\wp\\db\\WpdbRowMaterialization::hasFetchedRow\(\s*\$row\s*\)/.test(shell);
  const shellUsesRowValuesHelper = /\\wphx\\wp\\db\\WpdbRowMaterialization::rowValues\(/.test(shell);
  const shellUsesRowFieldsHelper = /\\wphx\\wp\\db\\WpdbRowMaterialization::rowFields\(/.test(shell);
  const shellUsesFirstFieldHelper = /\\wphx\\wp\\db\\WpdbRowMaterialization::firstFieldValue\(\s*\$row\s*\)/.test(shell);
  const rawObjectVarsInWpdbShell = /get_object_vars\s*\(/.test(shell);
  const rawArrayValuesObjectVarsInWpdbShell = /array_values\s*\(\s*get_object_vars\s*\(/.test(shell);
  const rawObjectKKeyMaterializationInShell = /\$var_by_ref\s*=\s*get_object_vars\s*\(\s*\$row\s*\)|\$key\s*=\s*array_shift\s*\(\s*\$var_by_ref\s*\)/.test(shell);
  const rawRowPresenceInShell = /is_object\s*\(\s*\$row\s*\)/.test(shell);
  const passed =
    directIsObject &&
    directGetObjectVars &&
    directArrayValues &&
    directArrayShift &&
    shellUsesPresenceHelper &&
    shellUsesRowValuesHelper &&
    shellUsesRowFieldsHelper &&
    shellUsesFirstFieldHelper &&
    !rawObjectVarsInWpdbShell &&
    !rawArrayValuesObjectVarsInWpdbShell &&
    !rawObjectKKeyMaterializationInShell &&
    !rawRowPresenceInShell;
  return {
    id: "wphx-305-21-live-wpdb-row-materialization",
    generated_haxe_materialization: {
      path: ROW_MATERIALIZATION_PHP,
      bytes: statSync(ROW_MATERIALIZATION_PHP).size,
      sha256: sha256File(ROW_MATERIALIZATION_PHP),
      evidence_lines: sourceLinesWithNeedles(materialization, ["is_object", "get_object_vars", "array_values", "array_shift"]),
      direct_is_object_call_detected: directIsObject,
      direct_get_object_vars_call_detected: directGetObjectVars,
      direct_array_values_call_detected: directArrayValues,
      direct_array_shift_call_detected: directArrayShift
    },
    transformed_wpdb_shell: {
      path: shellPath,
      bytes: statSync(shellPath).size,
      sha256: sha256File(shellPath),
      evidence_lines: sourceLinesWithNeedles(shell, ["WpdbRowMaterialization", "get_object_vars", "array_values", "array_shift", "is_object( $row"]),
      shell_uses_presence_helper: shellUsesPresenceHelper,
      shell_uses_row_values_helper: shellUsesRowValuesHelper,
      shell_uses_row_fields_helper: shellUsesRowFieldsHelper,
      shell_uses_first_field_helper: shellUsesFirstFieldHelper,
      raw_object_vars_in_wpdb_shell: rawObjectVarsInWpdbShell,
      raw_array_values_get_object_vars_in_wpdb_shell: rawArrayValuesObjectVarsInWpdbShell,
      raw_object_k_key_materialization_in_shell: rawObjectKKeyMaterializationInShell,
      raw_row_presence_in_shell: rawRowPresenceInShell
    },
    selected_strategy: "typed-haxe-row-materialization",
    passed
  };
}

async function withDbRuntime(runtime, callback) {
  const name = `wordpresshx-wphx-305-21-${runtime.id}-${process.pid}`;
  let containerId = "";
  try {
...
function writeOrCheck(path, contents) {
  if (checkOnly) {
    ...
    if (current !== contents) {
      throw new Error(`${path} is stale; run npm run wp:core:wphx-305-row-materialization-candidate`);
    }
    return;
  }
...
function ownershipManifest(manifestSha, upstreamDigest, runtimes) {
  return {
    ...
    manifest_id: "ownership:wp-core/wpdb-row-materialization-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      kind: "module",
      name: "wpdb row materialization live database Haxe candidate",
      area: "wp-includes/wp-admin",
      public_contract:
        "WordPress-compatible maybe_create_table(), maybe_add_column(), dbDelta(), and wpdb::query/get_var/get_row/get_results remain callable with their public PHP class/function shape while selected schema, dbDelta, result-population, raw mysqli metadata, query/fetch call gates, fetched-row traversal decisions, row field/value materialization, and mysqli_query()/mysqli_fetch_object() execution delegate to typed Haxe. The WPHX_305_17_MysqliBoundary generated PHP class is removed from this candidate; generated Haxe PHP emits direct global \\mysqli_query($handle, $query), \\mysqli_fetch_object($result), \\get_object_vars($row), \\array_values(...), \\array_shift(...), and \\is_object($row) calls through @:phpGlobal externs."
    },
    ...
    owned_paths: [...HAXE_SOURCES, "tools/wp-core/run-wpdb-row-materialization-candidate.mjs", OUT, RECEIPT],
    generated_paths: [OUT_ROOT, OUT, OWNERSHIP, RECEIPT],
    bridge: {
      kind: "generated_shell",
      reason:
        "WordPress core and plugins call global PHP upgrade helpers and wpdb methods with native PHP signatures, mutable public state, and stdClass/array result contracts. The candidate keeps those public PHP shells, delegates normalized table/column existence, dbDelta planning, selected-row result population, raw mysqli metadata guards, query/fetch call gates, fetched-row traversal policy, row field/value materialization, and mysqli query/fetch execution to typed Haxe, with WpdbMysqliExecution and WpdbRowMaterialization lowering to idiomatic PHP globals through @:phpGlobal externs.",
      bounded_by: [
        "generated candidate wp-admin/includes/upgrade.php shell",
        "generated candidate wp-includes/class-wpdb.php method shell",
        "WPHX-305.21 live MySQL/MariaDB oracle comparison receipt",
        "typed WpdbMysqliCallGate query/fetch guards",
        "typed WpdbMysqliExecution generated PHP helper",
        "typed WpdbRowMaterialization generated PHP helper",
        "WPHX-305.20 mysqli @:phpGlobal live candidate receipt",
        "WPHX-305.19 @:phpGlobal mysqli lowering proof"
      ]
    },
    removal_gate: {
      condition:
    ...
    smell_fixes: [
      {
        description:
          "Separated maybe_create_table()/maybe_add_column() branch decisions, bounded dbDelta classification, wpdb result-shape/null decisions, wpdb::query return/state branch decisions, _do_query timing/logging/execution/reconnect policy, selected-row last_result/num_rows population policy, fetched-row presence and assignment-slot policy, row field/value materialization, mysqli metadata read/fallback guards, mysqli query/fetch call gates, and mysqli query/fetch execution from handwritten/generated PHP boundaries into typed Haxe helpers. WPHX-305.21 keeps the two-method WPHX_305_17_MysqliBoundary class out of the live candidate and moves get_object_vars()/array_values()/array_shift()/is_object() row operations into generated Haxe PHP direct global calls.",
        compatibility_evidence: ["schema:maybe-create-table", "schema:maybe-add-column", "schema:dbdelta-plan-no-execute", "schema:dbdelta-execute-and-dml", "runtime:wpdb-result-shapes", "runtime:wpdb-native-execution-decisions", "runtime:wpdb-native-execution-log", "runtime:wpdb-mysqli-row-traversal-loop", "runtime:wpdb-mysqli-row-traversal-metadata"],
        behavior_policy: "no_observable_change"
      }
    ],
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-305-row-materialization-candidate",
    ...
    notes:
      `The candidate provisions ${runtimes.map((runtime) => runtime.id).join(" and ")} containers from locked index digests and runs the WPHX-305.07 live DB cases plus WPHX-305.11 result-shape, WPHX-305.12 query-state, WPHX-305.13 native-execution, WPHX-305.14 result-population, WPHX-305.15 raw-resource metadata, WPHX-305.16 boundary-entry, WPHX-305.17 fetched-row traversal, WPHX-305.19 mysqli @:phpGlobal lowering, WPHX-305.20 live @:phpGlobal integration, and WPHX-305.21 row materialization cases with local PHP mysqli. The broad wp-admin include remains stubbed because this workset is scoped to database helpers, dbDelta planning decisions, wpdb mysqli native execution, row result materialization, and storage paths.`
  };
}
...
const mysqliRowTraversalCandidate = readJson(MYSQLI_ROW_TRAVERSAL_CANDIDATE);
const mysqliGlobalLoweringProof = readJson(MYSQLI_GLOBAL_LOWERING_PROOF);
...
if (!maybeCommand("docker", ["info", "--format", "{{.ServerVersion}}"])) {
  console.error(JSON.stringify({ status: "failed", error: "docker server unavailable; WPHX-305.21 requires live DB containers" }, null, 2));
  process.exit(1);
}
...
writeProbe();
const mysqliIntegration = candidateMysqliIntegration();
const rowMaterialization = candidateRowMaterialization();

if (!mysqliIntegration.passed) {
  console.error(JSON.stringify({ status: "failed", error: "mysqli @:phpGlobal candidate integration shape failed", mysqliIntegration }, null, 2));
  process.exit(1);
}

if (!rowMaterialization.passed) {
  console.error(JSON.stringify({ status: "failed", error: "wpdb row materialization candidate shape failed", rowMaterialization }, null, 2));
  process.exit(1);
}
...
const manifest = {
  schema: "wphx.wp-core-wpdb-row-materialization-candidate.v1",
  issue: ISSUE.external_ref,
  generated_at: RECORDED_AT,
  generator: "tools/wp-core/run-wpdb-row-materialization-candidate.mjs",
  inputs: {
    ...
    mysqli_row_traversal_candidate_manifest: inputRecord(MYSQLI_ROW_TRAVERSAL_CANDIDATE),
    mysqli_global_lowering_proof_manifest: inputRecord(MYSQLI_GLOBAL_LOWERING_PROOF),
  ...
  fixture: {
    candidate_kind: "haxe_generated_wpdb_row_materialization_shell",
    ...
    compiler_lowering_evidence: {
      inherited_proof: {
        manifest: MYSQLI_GLOBAL_LOWERING_PROOF,
        validation_result: mysqliGlobalLoweringProof.validation_result,
        selected_integration_path: mysqliGlobalLoweringProof.selected_integration_path
      },
      live_candidate_integration: mysqliIntegration,
      live_row_materialization: rowMaterialization
    },
    ...
    inherited_mysqli_boundary_candidate: {
      manifest: MYSQLI_BOUNDARY_CANDIDATE,
      validation_result: mysqliBoundaryCandidate.validation_result
    },
    inherited_mysqli_row_traversal_candidate: {
      manifest: MYSQLI_ROW_TRAVERSAL_CANDIDATE,
      validation_result: mysqliRowTraversalCandidate.validation_result
    },
    inherited_mysqli_global_lowering_proof: {
      manifest: MYSQLI_GLOBAL_LOWERING_PROOF,
      validation_result: mysqliGlobalLoweringProof.validation_result
    },
    ...
    public_abi_policy: {
      ...
      raw_mysqli_calls_isolated_in_generated_php_boundary: false,
      typed_phpglobal_mysqli_calls_in_haxe_generated_php: true,
      generated_mysqli_boundary_removed_from_candidate_shell: true,
      fetched_row_traversal_decisions_are_typed_haxe: true,
      fetched_row_object_presence_is_typed_haxe: true,
      row_field_value_materialization_is_typed_haxe: true,
      raw_get_object_vars_removed_from_wpdb_shell: !rowMaterialization.transformed_wpdb_shell.raw_object_vars_in_wpdb_shell
    },
    live_boundaries_covered: [
      ...
      "wpdb::query fetched-row presence, break, last_result assignment-slot, and count-advance decisions through typed Haxe",
      "wpdb::get_var/get_row/get_results row field/value materialization through typed Haxe WpdbRowMaterialization",
      "wpdb::query mysqli errno/error/affected-row/insert-id metadata guard behavior over live rows",
      "wpdb::_do_query mysqli_query execution through typed Haxe WpdbMysqliExecution::query and generated \\mysqli_query($handle, $query)",
      "wpdb::query mysqli_fetch_object execution through typed Haxe WpdbMysqliExecution::fetchObject and generated \\mysqli_fetch_object($result)",
    ...
    native_boundaries: [
      ...
      {
        id: "typed-haxe-phpglobal-mysqli-execution",
        reason:
          "WPHX-305.19 proved that @:phpGlobal typed mysqli externs lower through the stock Haxe PHP target to direct global \\mysqli_query($handle, $query) and \\mysqli_fetch_object($result) calls. WPHX-305.21 integrates that generated Haxe helper into the live wpdb shell and removes the generated WPHX_305_17_MysqliBoundary class from this candidate."
      },
      {
        id: "typed-haxe-row-materialization",
        reason:
          "WPHX-305.21 routes fetched-row object presence, stdClass field extraction, numeric value extraction, and OBJECT_K first-field key materialization through generated Haxe PHP WpdbRowMaterialization helpers. Those helpers lower PHP-native row operations through typed @:phpGlobal externs instead of php.Syntax.code or generated PHP postprocessing."
      },
      {
    ...
    closes_gaps_from: [
      {
        ...
        resolution:
          "WPHX-305.21 extends the typed Haxe schema-helper/dbDelta candidate with bounded wpdb mysqli row traversal and row materialization policy while retaining live MySQL/MariaDB execution through wpdb."
      },
      {
        manifest: LIVE_DB_CANDIDATE,
        gap: "wpdb-class-not-yet-haxe-owned",
        resolution:
          "WPHX-305.21 narrows a bounded wpdb query result traversal and row materialization portion of this gap by routing mysqli_query()/mysqli_fetch_object() plus stdClass field/value extraction through typed Haxe helpers while preserving the WordPress-visible wpdb shell. Full wpdb replacement/drop-in behavior and storage ownership remain split to later WPHX-305 slices."
      },
      {
        manifest: DBDELTA_CANDIDATE,
        gap: "wpdb-class-not-yet-haxe-owned",
        resolution:
          "WPHX-305.21 keeps the dbDelta planning helper in the candidate while extending the wpdb class shell."
      },
      {
        manifest: QUERY_RESULT_CANDIDATE,
        gap: "wpdb-query-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.21 keeps the result-shape candidate in the live shell while routing selected-row traversal policy, row field/value materialization, and mysqli query/fetch execution through typed Haxe helpers."
      },
      {
        manifest: QUERY_STATE_CANDIDATE,
        gap: "wpdb-native-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.21 keeps the query-state candidate in the live shell while adding typed Haxe decisions for fetched-row presence, loop break, last_result assignment slot, and row-count advancement."
      },
      {
        manifest: NATIVE_EXECUTION_CANDIDATE,
        gap: "wpdb-native-execution-not-yet-haxe-owned",
        resolution:
          "WPHX-305.21 follows the native-execution candidate by keeping the WordPress-visible _do_query() shell while moving mysqli_query() execution into generated Haxe PHP through WpdbMysqliExecution::query()."
      },
      {
        manifest: RESULT_POPULATION_CANDIDATE,
        gap: "wpdb-raw-mysqli-resource-not-yet-haxe-owned",
        resolution:
          "WPHX-305.21 follows result-population ownership by keeping fetched-row loop break, last_result assignment-slot, count advancement policy, and stdClass row field/value materialization in typed Haxe around WpdbMysqliExecution::fetchObject()."
      },
      {
        manifest: RAW_RESOURCE_CANDIDATE,
        gap: "wpdb-raw-mysqli-query-result-fetch-not-yet-haxe-owned",
        resolution:
          "WPHX-305.21 narrows this gap with typed fetched-row traversal decisions, generated Haxe PHP direct global mysqli_query()/mysqli_fetch_object() calls, and typed Haxe-owned row field/value materialization."
      },
      {
        manifest: MYSQLI_BOUNDARY_CANDIDATE,
        gap: "wpdb-generated-mysqli-boundary-not-yet-replaced-by-direct-haxe-lowering",
        resolution:
          "WPHX-305.21 replaces the generated two-method mysqli boundary in the live candidate with WpdbMysqliExecution generated from typed Haxe @:phpGlobal externs."
      },
      {
        manifest: MYSQLI_ROW_TRAVERSAL_CANDIDATE,
        gap: "wpdb-generated-mysqli-boundary-not-yet-replaced-by-direct-haxe-lowering",
        resolution:
          "WPHX-305.21 preserves the WPHX-305.17 row traversal behavior while removing WPHX_305_17_MysqliBoundary from the candidate shell."
      },
      {
        manifest: MYSQLI_GLOBAL_LOWERING_PROOF,
        gap: "proof-not-yet-integrated-into-live-wpdb-candidate",
        resolution:
          "WPHX-305.21 integrates the WPHX-305.19 @:phpGlobal lowering proof into the live wpdb candidate and reruns the MySQL/MariaDB oracle comparison."
      },
      {
        manifest: MYSQLI_PHPGLOBAL_CANDIDATE,
        gap: "native-row-object-value-materialization-not-yet-typed",
        resolution:
          "WPHX-305.21 moves fetched-row object presence, get_var row value extraction, get_row ARRAY_A/ARRAY_N materialization, get_results ARRAY_A/ARRAY_N materialization, and OBJECT_K first-field key extraction into WpdbRowMaterialization generated from typed Haxe."
      }
    ],
    follows: ["WPHX-305.20", "WPHX-305.19", "WPHX-305.17", "WPHX-305.16", "WPHX-305.15", "WPHX-305.14", "WPHX-305.13", "WPHX-305.12", "WPHX-305.11", "WPHX-305.10", "WPHX-305.08", "WPHX-305.07", "WPHX-305.06"]
  },
  toolchain: {
  ...
  remaining_gaps: [
    {
      id: "full-wpdb-drop-in-and-public-state-ownership-not-yet-proven",
      owner: "future WPHX-305 wpdb distribution workset",
      detail: "This candidate keeps the public PHP wpdb method shell for plugin/drop-in ABI compatibility while moving another bounded query/result slice into typed Haxe. A later slice still needs to prove full wpdb replacement/drop-in loading, public mutable state ownership, and broader plugin compatibility."
    },
    {
  ...
  validation_result: {
    ...
    candidate_kind: "haxe_generated_wpdb_row_materialization_shell",
    selected_strategy: "typed-haxe-row-materialization",
    direct_query_call_detected: mysqliIntegration.generated_haxe_execution.direct_query_call_detected,
    direct_fetch_object_call_detected: mysqliIntegration.generated_haxe_execution.direct_fetch_object_call_detected,
    direct_row_is_object_call_detected: rowMaterialization.generated_haxe_materialization.direct_is_object_call_detected,
    direct_row_get_object_vars_call_detected: rowMaterialization.generated_haxe_materialization.direct_get_object_vars_call_detected,
    direct_row_array_values_call_detected: rowMaterialization.generated_haxe_materialization.direct_array_values_call_detected,
    direct_row_array_shift_call_detected: rowMaterialization.generated_haxe_materialization.direct_array_shift_call_detected,
    generated_mysqli_boundary_detected: mysqliIntegration.transformed_wpdb_shell.generated_boundary_detected,
    shell_uses_query_helper: mysqliIntegration.transformed_wpdb_shell.shell_uses_query_helper,
    shell_uses_fetch_helper: mysqliIntegration.transformed_wpdb_shell.shell_uses_fetch_helper,
    shell_uses_row_materialization_helper: rowMaterialization.transformed_wpdb_shell.shell_uses_presence_helper && rowMaterialization.transformed_wpdb_shell.shell_uses_row_values_helper && rowMaterialization.transformed_wpdb_shell.shell_uses_row_fields_helper && rowMaterialization.transformed_wpdb_shell.shell_uses_first_field_helper,
    raw_object_vars_in_wpdb_shell: rowMaterialization.transformed_wpdb_shell.raw_object_vars_in_wpdb_shell,
...
const receipt = {
  ...
  id: "receipt:wphx-305-21-wpdb-row-materialization-candidate",
  issue: ISSUE,
  recorded_at: RECORDED_AT,
  artifacts: [
    {
      path: OUT,
      role: "wpdb row materialization live MySQL/MariaDB Haxe candidate manifest"
    },
    {
      path: OWNERSHIP,
    ...
    {
      path: "tools/wp-core/run-wpdb-row-materialization-candidate.mjs",
      role: "wpdb row materialization live DB candidate generator and check-mode validator"
    },
    {
    ...
    {
      path: "src/wphx/wp/db/WpdbMysqliCallGate.hx",
      role: "typed Haxe wpdb mysqli query/fetch call gate decision implementation"
    },
    {
      path: "src/wphx/wp/db/WpdbMysqliExecution.hx",
      role: "typed Haxe wpdb mysqli @:phpGlobal execution implementation"
    },
    {
      path: "src/wphx/wp/db/native/MysqliGlobal.hx",
      role: "typed @:phpGlobal mysqli extern implementation"
    },
    {
      path: "src/wphx/wp/db/WpdbRowTraversal.hx",
      role: "typed Haxe wpdb fetched-row traversal decision implementation"
    },
    {
      path: "src/wphx/wp/db/WpdbRowMaterialization.hx",
      role: "typed Haxe wpdb fetched-row object and field/value materialization implementation"
    },
    {
      path: "src/wphx/wp/db/native/RowGlobal.hx",
      role: "typed @:phpGlobal row/stdClass/native-array extern implementation"
    }
  ],
  ...
  JSON.stringify(
    {
      ...
      direct_query_call_detected: manifest.validation_result.direct_query_call_detected,
      direct_fetch_object_call_detected: manifest.validation_result.direct_fetch_object_call_detected,
      direct_row_get_object_vars_call_detected: manifest.validation_result.direct_row_get_object_vars_call_detected,
      shell_uses_row_materialization_helper: manifest.validation_result.shell_uses_row_materialization_helper,
      generated_mysqli_boundary_detected: manifest.validation_result.generated_mysqli_boundary_detected,
      promoted_symbols: PROMOTED_SYMBOLS.length,
      promoted_decisions: PROMOTED_DECISIONS.length,
      covered_symbols: COVERED_SYMBOLS.length,
      fixture_cases: FIXTURE_CASES.length,
