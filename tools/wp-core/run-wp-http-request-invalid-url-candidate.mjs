...
const ISSUE = {
  id: "wordpresshx-46s",
  external_ref: "WPHX-312.74",
  title: "WPHX-312.74 - Promote WP_Http request invalid URL decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-invalid-url-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-invalid-url-candidate.hxml";
const WPHX_PHP_HXML = "fixtures/wphx-php/wp-http-request-nonblocking.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-74";
...
const OUT = "manifests/wp-core/wphx-312-74-wp-http-request-invalid-url-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-74-wp-http-request-invalid-url-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-74-wp-http-request-invalid-url-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestInvalidUrlCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestInvalidUrl\\HttpRequestInvalidUrl_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request empty URL or missing scheme invalid-url decision"];
const CASES = [
  { id: "wp-http-request:invalid-and-blocked", focus: "relative URL without scheme takes invalid-url error path while blocked absolute URL stays PHP-owned" }
];
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-invalid-url-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request invalid URL decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that an empty request URL or missing parsed URL scheme takes the invalid-url WP_Error branch. PHP still owns public WP_Http::request ABI, pre_http_request behavior, unsafe URL filtering, parse_url execution, WP_Error construction, translation, http_api_debug action payload/timing, block_request behavior, Requests dispatch avoidance, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-invalid-url-candidate",
        "npm run wp:core:wphx-312-wp-http-request-invalid-url-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run wphx:php:adapter-raw-blocks:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-74-wp-http-request-invalid-url-candidate"],
...
async function main() {
  ...
  const wphxPhpShape = {
    ...
    invalid_url_haxe_call: generatedShell.includes(`${HAXE_MODULE}::shouldRejectInvalidRequestUrl`),
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-invalid-url-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request invalid URL branch decision is claimed by this candidate. The surrounding public method body is now emitted by structured WPHX PHP Adapter IR as a bounded original-path adapter and remains PHP-owned request orchestration, not full request behavior ownership.",
      adapter_ir: {
    ...
    fixture: {
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted invalid-url decision is a typed URL/scheme-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-invalid-url-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the empty URL or missing parsed scheme branch decision is Haxe-owned. URL filtering/sanitization, parse_url execution, WP_Error construction, translation, debug actions, block_request policy, Requests dispatch avoidance, and all successful request option handoffs remain PHP-owned or unclaimed in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      url_parsing_claimed: false,
      url_validation_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-74-wp-http-request-invalid-url-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request invalid URL Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request invalid URL Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request invalid URL candidate" }
    ],
