...
function haxeBootstrapBlock() {
	return `if ( ! function_exists( 'wphx_312_74_bootstrap_haxe' ) ) {
\tfunction wphx_312_74_bootstrap_haxe() {
...
\t\t$wphx_312_74_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_74_lib );
...
function transformCandidateRequestInvalidUrl() {
	const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
	const original = `\t\tif ( empty( $url ) || empty( $parsed_url['scheme'] ) ) {
...
\t\t}
`;
	const replacement = `\t\tif ( ${HAXE_MODULE}::shouldRejectInvalidRequestUrl( (string) $url, $parsed_url['scheme'] ?? null ) ) {
\t\t\t$response = new WP_Error( 'http_request_failed', __( 'A valid URL was not provided.' ) );
\t\t\t/** This action is documented in wp-includes/class-wp-http.php */
\t\t\tdo_action( 'http_api_debug', $response, 'response', 'WpOrg\\Requests\\Requests', $parsed_args, $url );
\t\t\treturn $response;
...
`;
  ...
  if (transformed === source) throw new Error("Unable to replace WP_Http::request invalid URL decision branch");
...
async function main() {
  ...
  transformCandidateRequestInvalidUrl();
  writeProbe();
  ...
  const manifest = {
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request invalid URL branch decision is emitted by generated Haxe PHP. The surrounding public method body, URL filtering/parsing, WP_Error construction, debug action, block policy, and Requests dispatch remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
