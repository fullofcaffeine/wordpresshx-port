...
const ISSUE = {
  id: "wordpresshx-zaz",
  external_ref: "WPHX-312.72",
  title: "WPHX-312.72 - Promote WP_Http request stream blocking decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-stream-blocking-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-stream-blocking-candidate.hxml";
const WPHX_PHP_HXML = "fixtures/wphx-php/wp-http-request-nonblocking.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-72";
...
const OUT = "manifests/wp-core/wphx-312-72-wp-http-request-stream-blocking-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-72-wp-http-request-stream-blocking-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-72-wp-http-request-stream-blocking-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestStreamBlockingCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestStreamBlocking\\HttpRequestStreamBlocking_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request stream=true forces blocking decision"];
const CASES = [
  { id: "wp-http-request:streaming", focus: "stream=true forces Requests blocking option before dispatch" }
];
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-stream-blocking-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request stream blocking decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that stream=true forces the parsed blocking option to true before Requests dispatch. PHP still owns public WP_Http::request ABI, request defaults and filters, stream filename defaulting, writable-directory validation, header/cookie/redirect/SSL/max_bytes/data_format handoff, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-stream-blocking-candidate",
        "npm run wp:core:wphx-312-wp-http-request-stream-blocking-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-72-wp-http-request-stream-blocking-candidate"],
...
async function main() {
  ...
  const wphxPhpShape = {
    ...
    stream_blocking_haxe_call: generatedShell.includes(`${HAXE_MODULE}::shouldForceBlockingForStream`),
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-stream-blocking-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request stream blocking assignment decision is claimed by this candidate. The surrounding public method body is now emitted by structured WPHX PHP Adapter IR as a bounded original-path adapter and remains PHP-owned request orchestration, not full request behavior ownership.",
      adapter_ir: {
    ...
    fixture: {
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted stream-blocking decision is a pure bool-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-stream-blocking-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the stream=true forces blocking=true decision is Haxe-owned. Parsed argument defaults/filters, filename defaulting, writable-directory validation, hook registration, headers, cookies, redirects, SSL verification, max_bytes, data_format, Requests dispatch, response filtering, and actual streaming transfer behavior remain PHP-owned or unclaimed in this candidate."
      },
      {
      ...
      },
      {
        ...
        owner: ISSUE.external_ref,
        detail:
          "The candidate consumes a compiler-emitted original-path class-wp-http.php shell for a bounded WP_Http::request stream branch gate plus supporting request/block_request branches. Broader request semantics, whole-file WP_Http ownership, and installed distribution behavior remain later compiler-driven gates."
      }
    ],
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-72-wp-http-request-stream-blocking-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request stream blocking Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request stream blocking Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request stream blocking candidate" }
    ],
