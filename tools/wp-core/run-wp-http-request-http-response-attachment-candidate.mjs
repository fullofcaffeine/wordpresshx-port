...
const ISSUE = {
  id: "wordpresshx-3kf",
  external_ref: "WPHX-312.89",
  title: "WPHX-312.89 - Promote WP_Http request http_response attachment decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-http-response-attachment-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-http-response-attachment-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-89";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-89-wp-http-request-http-response-attachment-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-89-wp-http-request-http-response-attachment-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-89-wp-http-request-http-response-attachment-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestHttpResponseAttachmentCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestHttpResponseAttachment\\HttpRequestHttpResponseAttachment_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request successful response attaches original WP_HTTP_Requests_Response object"];
const CASES = [
  { id: "wp-http-request:head-defaults", focus: "successful blocking response attaches http_response object before http_response filter" }
];
...
function haxeBootstrapBlock() {
	return `if ( ! function_exists( 'wphx_312_89_bootstrap_haxe' ) ) {
\tfunction wphx_312_89_bootstrap_haxe() {
...
\t\t$wphx_312_89_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_89_lib );
...
function transformCandidateRequestHttpResponseAttachment() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\t\t// Add the original object to the array.
\t\t\t$response['http_response'] = $http_response;
`;
  const replacement = `\t\t\t// Add the original object to the array.
\t\t\tif ( ${HAXE_MODULE}::shouldAttachHttpResponseObject() ) {
\t\t\t\t$response['http_response'] = $http_response;
\t\t\t}
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request http_response attachment decision branch");
...
\t\t$assertions['http_response_object_attached'] = 'WP_HTTP_Requests_Response' === ( $result['response']['http_response']['class'] ?? null );
\t\tbreak;
...
\t\t$response = $http->request( 'https://example.test/files/archive.zip', array( 'stream' => true ) );
\t\t$stream_call = $GLOBALS['wphx_requests_calls'][0] ?? array();
\t\t$custom = $http->request( 'https://example.test/files/custom.zip', array( 'stream' => true, 'filename' => '/tmp/wphx-streams/custom.zip' ) );
\t\t$custom_call = $GLOBALS['wphx_requests_calls'][1] ?? array();
...
\t\t$result['custom'] = wphx_summarize( $custom );
\t\t$result['custom_call'] = $custom_call;
\t\t$result['bad'] = wphx_error_summary( $bad );
\t\t$assertions['default_filename'] = '/tmp/wphx-streams/archive.zip' === $stream_call['options']['filename'];
\t\t$assertions['custom_filename_preserved'] = '/tmp/wphx-streams/custom.zip' === $custom_call['options']['filename'];
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-http-response-attachment-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request http_response attachment decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that a successful Requests response attaches the original WP_HTTP_Requests_Response wrapper to the public response array under http_response before debug and response-filter handling. PHP still owns public WP_Http::request ABI, request defaults and filters, Requests dispatch, response wrapper construction, response array conversion, debug/filter timing, error and nonblocking exits, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-http-response-attachment-candidate",
        "npm run wp:core:wphx-312-wp-http-request-http-response-attachment-candidate:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-89-wp-http-request-http-response-attachment-candidate"],
...
async function main() {
  ...
  transformCandidateRequestHttpResponseAttachment();
  writeProbe();
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-http-response-attachment-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request successful http_response object attachment decision is emitted by generated Haxe PHP. The surrounding public method body, Requests dispatch, WP_HTTP_Requests_Response construction, response array conversion, debug timing, error/nonblocking exits, and response filtering remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
      ...
      native_boundary_policy: {
        native_php_array_shape_required: false,
        escape_hatch_owner: null,
        detail: "The promoted http_response attachment decision is a pure no-argument bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-http-response-attachment-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the decision to attach the original WP_HTTP_Requests_Response wrapper to a successful public response array is Haxe-owned. Parsed argument defaults/filters, hook registration, headers, cookies, redirects, SSL verification, max_bytes, data_format, Requests dispatch, response wrapper construction, response array conversion, debug timing, error and nonblocking exits, response filtering, and transport behavior remain PHP-owned or unclaimed in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      http_response_attachment_decision_claimed: true,
      response_wrapper_construction_claimed: false,
      response_filtering_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-89-wp-http-request-http-response-attachment-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request http_response attachment Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request http_response attachment Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request http_response attachment candidate" }
    ],
