...
const ISSUE = {
  id: "wordpresshx-l76.10.6",
  external_ref: "WPHX-311.06",
  title: "Promote REST server dispatch routing to Haxe candidate"
};
const HXML = "fixtures/wp-core/rest-server-dispatch-strategy-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-311-06";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-311-06-rest-server-dispatch-strategy-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-311-06-rest-server-dispatch-strategy-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-311-06-rest-server-dispatch-strategy-candidate.v1.json";
...
const UPDATE_CANDIDATE = "manifests/wp-core/wphx-311-05-rest-settings-update-strategy-candidate.v1.json";
const RECORDED_AT = "2026-06-22T11:25:00.000Z";
...
const PROMOTED_SYMBOLS = ["WP_REST_Server::dispatch", "WP_REST_Server::respond_to_request"];
...
function haxeBootstrapBlock(dirnameDepth) {
  return `if ( ! function_exists( 'wphx_311_06_bootstrap_haxe' ) ) {
\tfunction wphx_311_06_bootstrap_haxe() {
...
\t\t$wphx_311_06_lib = dirname( __DIR__, ${dirnameDepth} ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_311_06_lib );
...
function writeOrCheck(path, contents) {
  if (checkOnly) {
    ...
    if (current !== contents) {
      throw new Error(`${path} is stale; run npm run wp:core:wphx-311-rest-server-dispatch-candidate`);
    }
    return;
  }
...
function ownershipManifest(manifestSha, upstreamDigest) {
  return {
    ...
    manifest_id: "ownership:wp-core/rest-server-dispatch-strategy-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_REST_Server dispatch routing strategy",
      area: "wp-includes/rest-api/class-wp-rest-server.php",
      public_contract:
        "WP_REST_Server::dispatch() and WP_REST_Server::respond_to_request() must preserve the public PHP ABI and plugin filter/callback behavior while typed Haxe owns the dispatch routing decisions."
    },
    ...
    owned_paths: [
      ...HAXE_SOURCES,
      "tools/wp-core/run-rest-server-dispatch-strategy-candidate.mjs",
      OUT,
      OWNERSHIP,
      RECEIPT
    ],
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-311-rest-server-dispatch-candidate",
    ...
    notes:
      "This is a linked candidate over mirrored WordPress source. It does not yet prove HTTP serving, installed distribution packaging, or full REST server replacement without upstream PHP source present."
  };
}
...
command("php", ["-l", `${HAXE_OUT}/lib/wphx/wp/rest/RestServerDispatchStrategy.php`]);
...
if (!sourceEscapeAuditPassed) {
  console.error(JSON.stringify({ status: "failed", reason: "haxe source escape audit failed", haxeSourceAudits }, null, 2));
  process.exit(1);
}
...
const manifest = {
  schema: "wphx.wp-core-rest-server-dispatch-strategy-candidate.v1",
  issue: ISSUE.external_ref,
  generated_at: RECORDED_AT,
  generator: "tools/wp-core/run-rest-server-dispatch-strategy-candidate.mjs",
  inputs: {
    ...
    update_candidate_manifest: inputRecord(UPDATE_CANDIDATE),
  ...
  fixture: {
    candidate_kind: "haxe_generated_rest_server_dispatch_strategy_shell",
    ...
    haxe_strategy_probe: {
      ...
      route: "typed_haxe_rest_server_dispatch_plan",
      owned_methods: ["dispatch", "respond_to_request"]
    },
    native_boundaries: [
      {
      ...
      {
        id: "route-registry-and-matching-native-boundary",
        reason:
          "Route registration, route matching table shape, PHP callbacks, and request/response object ABI remain native WordPress PHP surfaces in this linked candidate."
      }
    ],
  ...
  generated: {
    haxe_out: HAXE_OUT,
    files: filesUnder(HAXE_OUT).map((file) => ({
      path: `${HAXE_OUT}/${file.path}`,
      bytes: file.bytes,
      sha256: `sha256:${file.sha256}`
    })),
    candidate_server: inputRecord(`${CANDIDATE_ROOT}/wp-includes/rest-api/class-wp-rest-server.php`),
    strategy_php: inputRecord(`${HAXE_OUT}/lib/wphx/wp/rest/RestServerDispatchStrategy.php`),
    php_lint: {
      candidate_server: "passed",
      generated_strategy: "passed"
    }
  },
  ...
  remaining_gaps: [
    {
      ...
      owner: "WPHX-311",
      detail:
        "WP_REST_Server::serve_request(), headers, CORS, body parsing, JSON response emission, and web-server integration are outside this internal dispatch candidate."
    },
    {
      id: "packaged-distribution-deferred",
      owner: "WPHX-311",
      detail:
        "The candidate is installed into a mirrored source tree. A packaged distribution test with upstream class-wp-rest-server.php replacement and no fallback remains future work."
    },
    {
...
const receipt = {
  ...
  id: "receipt:wphx-311-06-rest-server-dispatch-strategy-candidate",
  issue: ISSUE,
  recorded_at: RECORDED_AT,
  artifacts: [
    {
      path: OUT,
      role: "REST server dispatch typed Haxe candidate parity manifest"
    },
    {
      path: OWNERSHIP,
      role: "linked-candidate ownership manifest for the typed dispatch strategy"
    },
    {
      path: "tools/wp-core/run-rest-server-dispatch-strategy-candidate.mjs",
      role: "candidate generator and check-mode validator"
    },
    {
      path: "src/wphx/wp/rest/RestServerDispatchStrategy.hx",
      role: "typed Haxe dispatch decision strategy"
    }
  ],
