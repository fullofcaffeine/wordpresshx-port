...
const ISSUE = {
  id: "wordpresshx-kzx",
  external_ref: "WPHX-312.78",
  title: "WPHX-312.78 - Promote WP_Http request proxy option decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T05:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-proxy-options-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-proxy-options-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-78";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-312-78-wp-http-request-proxy-options-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-78-wp-http-request-proxy-options-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-78-wp-http-request-proxy-options-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestProxyOptionsCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestProxyOptions\\HttpRequestProxyOptions_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request proxy enabled and send-through decision"];
const CASES = [
...
function haxeBootstrapBlock() {
  return `if ( ! function_exists( 'wphx_312_78_bootstrap_haxe' ) ) {
\tfunction wphx_312_78_bootstrap_haxe() {
...
\t\t$wphx_312_78_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_78_lib );
...
function transformCandidateRequestProxyOptions() {
  const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
  const original = `\t\t$proxy = new WP_HTTP_Proxy();
\t\tif ( $proxy->is_enabled() && $proxy->send_through_proxy( $url ) ) {
\t\t\t$options['proxy'] = new WpOrg\\Requests\\Proxy\\Http( $proxy->host() . ':' . $proxy->port() );
`;
  const replacement = `\t\t$proxy = new WP_HTTP_Proxy();
\t\t$wphx_proxy_enabled = $proxy->is_enabled();
\t\t$wphx_send_through_proxy = $wphx_proxy_enabled ? $proxy->send_through_proxy( $url ) : false;
\t\tif ( ${HAXE_MODULE}::shouldUseProxy( (bool) $wphx_proxy_enabled, (bool) $wphx_send_through_proxy ) ) {
\t\t\t$options['proxy'] = new WpOrg\\Requests\\Proxy\\Http( $proxy->host() . ':' . $proxy->port() );
`;
  const source = installBootstrap(readFileSync(path, "utf8"));
  const transformed = source.replace(original, replacement);
  if (transformed === source) throw new Error("Unable to replace WP_Http::request proxy option condition");
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-proxy-options-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request proxy option decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that a Requests proxy option is attached when WP_HTTP_Proxy is enabled and send_through_proxy returns true. PHP still owns public request ABI, WP_HTTP_Proxy policy and filters, proxy constants, host/bypass matching, authentication handoff, native WpOrg Requests Proxy Http object construction, Requests dispatch, debug/error handling, and native arrays/objects."
    },
    ...
    bridge: {
      ...
      removal_gate:
        "Replace copied public PHP with generated original-path adapters and pass request proxy option, selected upstream HTTP PHPUnit, installed distribution, and live/recorded network/proxy parity gates before claiming public PHP ownership."
    },
    owned_paths: [RUNNER, OUT, OWNERSHIP, RECEIPT],
    generated_paths: [OUT, OWNERSHIP, RECEIPT, OUT_ROOT],
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-proxy-options-candidate",
        "npm run wp:core:wphx-312-wp-http-request-proxy-options-candidate:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-78-wp-http-request-proxy-options-candidate"],
...
async function main() {
  ...
  transformCandidateRequestProxyOptions();
  writeProbe();
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-proxy-options-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the proxy enabled plus send-through branch decision in WP_Http::request is emitted by generated Haxe PHP. WP_HTTP_Proxy policy, pre_http_send_through_proxy filtering, proxy object construction, authentication property handoff, Requests dispatch, and response handling remain PHP-owned."
    },
    fixture: {
      cases: CASES,
      ...
      side_effect_policy: {
        ...
        php_cli: true,
        runtime_stubs:
          "Requests classes, selected WordPress globals, hook dispatch, site URL, proxy constants, mbstring encoding guards, and option/bloginfo helpers are deterministic stubs. Copied WP_Http and HTTP support classes remain the executed request proxy option sources; outbound Requests::request records options without network I/O."
      },
      public_abi_policy: {
    ...
    remaining_gaps: [
      {
        id: "full-request-proxy-options-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the proxy enabled plus send-through branch decision is Haxe-owned. WP_HTTP_Proxy enablement, host/bypass matching, pre_http_send_through_proxy filtering, proxy address/authentication handoff, native Requests proxy object behavior, Requests dispatch, response filtering, and actual proxy negotiation remain PHP-owned or unclaimed in this candidate."
      },
      {
    ...
    validation_result: {
      ...
      public_php_replacement_claimed: false,
      full_request_proxy_options_claimed: false,
      wp_http_proxy_policy_claimed: false,
      proxy_authentication_handoff_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-78-wp-http-request-proxy-options-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request proxy options Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request proxy options Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request proxy options candidate" }
    ],
