...
const ISSUE = {
  id: "wordpresshx-24b",
  external_ref: "WPHX-312.73",
  title: "WPHX-312.73 - Promote WP_Http request stream filename options decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-stream-filename-options-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-stream-filename-options-candidate.hxml";
const WPHX_PHP_HXML = "fixtures/wphx-php/wp-http-request-nonblocking.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-73";
...
const OUT = "manifests/wp-core/wphx-312-73-wp-http-request-stream-filename-options-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-73-wp-http-request-stream-filename-options-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-73-wp-http-request-stream-filename-options-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestStreamFilenameOptionsCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestStreamFilenameOptions\\HttpRequestStreamFilenameOptions_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request stream=true sets filename option decision"];
const CASES = [
  { id: "wp-http-request:streaming", focus: "stream=true sets Requests filename option after filename resolution" }
];
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-stream-filename-options-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request stream filename options decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that stream=true sets the Requests filename option from the already-resolved parsed filename. PHP still owns public WP_Http::request ABI, request defaults and filters, stream filename defaulting, writable-directory validation, forced blocking behavior, header/cookie/redirect/SSL/max_bytes/data_format handoff, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    bridge: {
      ...
      kind: "none",
      removal_gate:
        "Temporary copied public PHP shell retired for this bounded branch; full WP_Http::request ownership still requires request orchestration, selected upstream HTTP PHPUnit, installed distribution, and live/recorded network parity gates."
    },
    owned_paths: [RUNNER, OUT, OWNERSHIP, RECEIPT],
    generated_paths: [OUT, OWNERSHIP, RECEIPT, OUT_ROOT],
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-stream-filename-options-candidate",
        "npm run wp:core:wphx-312-wp-http-request-stream-filename-options-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run wphx:php:adapter-raw-blocks:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-73-wp-http-request-stream-filename-options-candidate"],
...
async function main() {
  ...
  const wphxPhpShape = {
    ...
    stream_filename_options_haxe_call: generatedShell.includes(`${HAXE_MODULE}::shouldSetStreamFilenameOption`),
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-stream-filename-options-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request stream filename option assignment decision is claimed by this candidate. The surrounding public method body is now emitted by structured WPHX PHP Adapter IR as a bounded original-path adapter and remains PHP-owned request orchestration, not full request behavior ownership.",
      adapter_ir: {
    ...
    fixture: {
      ...
      side_effect_policy: {
        ...
        php_cli: true,
        runtime_stubs:
          "Requests classes, selected WordPress globals, hook dispatch, URL validation, temp-dir writability, mbstring encoding guards, and option/bloginfo helpers are deterministic stubs. The oracle root executes copied upstream WP_Http while the candidate root executes a WPHX compiler-emitted original-path WP_Http shell; outbound Requests::request records options or throws without network I/O."
      },
      public_abi_policy: {
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted stream-filename-options decision is a pure bool-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-stream-filename-options-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the stream=true sets the Requests filename option decision is Haxe-owned. Parsed argument defaults/filters, filename defaulting, writable-directory validation, forced blocking behavior, hook registration, headers, cookies, redirects, SSL verification, max_bytes, data_format, Requests dispatch, response filtering, and actual streaming transfer behavior remain PHP-owned or unclaimed in this candidate."
      },
      {
      ...
      },
      {
        ...
        owner: ISSUE.external_ref,
        detail:
          "The candidate consumes a compiler-emitted original-path class-wp-http.php shell for a bounded WP_Http::request stream-filename branch gate plus supporting request/block_request branches. Broader request semantics, whole-file WP_Http ownership, and installed distribution behavior remain later compiler-driven gates."
      }
    ],
    ...
    validation_result: {
      ...
      streaming_file_io_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-73-wp-http-request-stream-filename-options-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request stream filename options Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request stream filename options Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request stream filename options candidate" },
      { path: WPHX_PHP_HXML, role: "WPHX PHP compiler target for the generated original-path request shell" }
    ],
