...
const ISSUE = {
  id: "wordpresshx-819",
  external_ref: "WPHX-312.75",
  title: "WPHX-312.75 - Promote WP_Http request blocked request decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-blocked-request-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-blocked-request-candidate.hxml";
const WPHX_PHP_HXML = "fixtures/wphx-php/wp-http-request-nonblocking.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-75";
...
const OUT = "manifests/wp-core/wphx-312-75-wp-http-request-blocked-request-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-75-wp-http-request-blocked-request-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-75-wp-http-request-blocked-request-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestBlockedRequestCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestBlockedRequest\\HttpRequestBlockedRequest_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request true block_request result returns blocked-request error decision"];
const CASES = [
  { id: "wp-http-request:invalid-and-blocked", focus: "blocked absolute URL takes blocked-request error path while invalid relative URL stays PHP-owned" }
];
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-blocked-request-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request blocked request decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that a true WP_Http::block_request result takes the blocked-request WP_Error branch. PHP still owns public WP_Http::request ABI, URL filtering/parsing, invalid-url handling, WP_Http::block_request execution and policy, WP_Error construction, translation/sprintf, http_api_debug action payload/timing, Requests dispatch avoidance, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-blocked-request-candidate",
        "npm run wp:core:wphx-312-wp-http-request-blocked-request-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run wphx:php:adapter-raw-blocks:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-75-wp-http-request-blocked-request-candidate"],
...
async function main() {
  ...
  const wphxPhpShape = {
    ...
    blocked_request_haxe_call: generatedShell.includes(`${HAXE_MODULE}::shouldReturnBlockedRequestError`),
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-blocked-request-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request blocked-request branch decision over the PHP-owned block_request result is claimed by this candidate. The surrounding public method body is now emitted by structured WPHX PHP Adapter IR as a bounded original-path adapter and remains PHP-owned request orchestration, not full request behavior ownership.",
      adapter_ir: {
    ...
    fixture: {
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted blocked-request decision is a pure bool-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-blocked-request-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the true block_request result branch decision is Haxe-owned. URL filtering/sanitization, parse_url execution, invalid-url handling, block_request internals and host policy, WP_Error construction, translation/sprintf, debug actions, Requests dispatch avoidance, and all successful request option handoffs remain PHP-owned or unclaimed in this candidate."
      },
      {
      ...
      },
      {
        ...
        owner: ISSUE.external_ref,
        detail: "The candidate now executes a compiler-emitted original-path WP_Http::request shell for this bounded branch, but full request orchestration, whole-file WP_Http ownership, selected upstream HTTP PHPUnit parity, and installed-distribution behavior remain later gates."
      }
    ],
    ...
    validation_result: {
      ...
      block_request_policy_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-75-wp-http-request-blocked-request-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request blocked request Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request blocked request Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request blocked request candidate" }
    ],
