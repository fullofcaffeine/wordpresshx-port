...
function haxeBootstrapBlock() {
	return `if ( ! function_exists( 'wphx_312_75_bootstrap_haxe' ) ) {
\tfunction wphx_312_75_bootstrap_haxe() {
...
\t\t$wphx_312_75_lib = dirname( __DIR__, 2 ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_312_75_lib );
...
function transformCandidateRequestBlockedRequest() {
	const path = `${CANDIDATE_ROOT}/wp-includes/class-wp-http.php`;
	const original = `\t\tif ( $this->block_request( $url ) ) {
...
\t\t}
`;
	const replacement = `\t\tif ( ${HAXE_MODULE}::shouldReturnBlockedRequestError( (bool) $this->block_request( $url ) ) ) {
\t\t\t/* translators: %s: URL to which the HTTP request was blocked. */
\t\t\t$response = new WP_Error( 'http_request_not_executed', sprintf( __( 'User has blocked requests through HTTP to the URL: %s.' ), $url ) );
\t\t\t/** This action is documented in wp-includes/class-wp-http.php */
\t\t\tdo_action( 'http_api_debug', $response, 'response', 'WpOrg\\Requests\\Requests', $parsed_args, $url );
...
`;
  ...
  if (transformed === source) throw new Error("Unable to replace WP_Http::request blocked request decision branch");
...
async function main() {
  ...
  transformCandidateRequestBlockedRequest();
  writeProbe();
  ...
  const manifest = {
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request blocked-request branch decision over the PHP-owned block_request result is emitted by generated Haxe PHP. The surrounding public method body, URL filtering/parsing, block_request policy, WP_Error construction, debug action, and Requests dispatch remain a copied candidate shell."
    },
    fixture: {
      cases: CASES,
