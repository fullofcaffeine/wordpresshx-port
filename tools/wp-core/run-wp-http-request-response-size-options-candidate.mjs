...
const ISSUE = {
  id: "wordpresshx-4xb",
  external_ref: "WPHX-312.71",
  title: "WPHX-312.71 - Promote WP_Http request response-size option decision to Haxe candidate"
};
const RECORDED_AT = "2026-06-28T08:00:00.000Z";
const UPSTREAM_ROOT = "../wordpress-develop";
const RUNNER = "tools/wp-core/run-wp-http-request-response-size-options-candidate.mjs";
const HXML = "fixtures/wp-core/http-request-response-size-options-candidate.hxml";
const WPHX_PHP_HXML = "fixtures/wphx-php/wp-http-request-nonblocking.hxml";
const OUT_ROOT = "build/wp-core/wphx-312-71";
...
const OUT = "manifests/wp-core/wphx-312-71-wp-http-request-response-size-options-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-312-71-wp-http-request-response-size-options-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-312-71-wp-http-request-response-size-options-candidate.v1.json";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/HttpRequestResponseSizeOptionsCandidateEntry.hx"
];
const HAXE_MODULE = "\\wphx\\wp\\http\\_HttpRequestResponseSizeOptions\\HttpRequestResponseSizeOptions_Fields_";
const PROMOTED_SYMBOLS = ["WP_Http::request non-null limit_response_size sets max_bytes decision"];
const CASES = [
  { id: "wp-http-request:head-defaults", focus: "default null limit_response_size omits Requests max_bytes" },
  { id: "wp-http-request:headers-cookies-ssl-post", focus: "non-null limit_response_size flows to Requests max_bytes" }
];
...
function ownershipManifest(manifestSha) {
  return {
    ...
    manifest_id: "ownership:wp-core/wp-http-request-response-size-options-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_Http request response-size option decision",
      area:
      ...
      public_contract:
        "This candidate promotes only the WP_Http::request decision that a non-null limit_response_size sets the Requests max_bytes option while the default null value omits max_bytes. PHP still owns public WP_Http::request ABI, request defaults and filters, header/cookie/redirect/SSL/data_format handoff, streaming filename behavior, Requests dispatch, response filtering, and native arrays/objects."
    },
    ...
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-312-wp-http-request-response-size-options-candidate",
        "npm run wp:core:wphx-312-wp-http-request-response-size-options-candidate:check",
        "npm run wphx:php:public-shell-snapshots:check",
        "npm run wphx:php:adapter-raw-blocks:check",
        "npm run receipts:validate",
        "npm run beads:validate"
      ],
      receipt_refs: ["receipt:wphx-312-71-wp-http-request-response-size-options-candidate"],
...
async function main() {
  ...
  const wphxPhpShape = {
    ...
    response_size_options_haxe_call: generatedShell.includes(`${HAXE_MODULE}::shouldSetMaxBytes`),
  ...
  const manifest = {
    schema: "wphx.wp-core-wp-http-request-response-size-options-candidate.v1",
    ...
    candidate: {
      ...
      promoted_behavior:
        "Only the WP_Http::request response-size option branch decision is claimed by this candidate. The surrounding public method body is now emitted by structured WPHX PHP Adapter IR as a bounded original-path adapter and remains PHP-owned request orchestration, not full request behavior ownership.",
      adapter_ir: {
    ...
    fixture: {
      ...
      native_boundary_policy: {
        ...
        detail: "The promoted response-size decision is a pure nullable-int-to-bool Haxe function and does not require a target-native escape hatch."
      }
    },
    ...
    remaining_gaps: [
      {
        id: "full-request-response-size-options-not-promoted",
        owner: ISSUE.external_ref,
        detail:
          "Only the non-null limit_response_size branch decision is Haxe-owned. Parsed argument defaults/filters, hook registration, headers, cookies, redirects, SSL verification, data_format, Requests dispatch, response filtering, and actual response truncation behavior remain PHP-owned or unclaimed in this candidate."
      },
      {
      ...
      },
      {
        ...
        owner: ISSUE.external_ref,
        detail:
          "The candidate consumes a compiler-emitted original-path class-wp-http.php shell for a bounded WP_Http::request response-size branch gate plus supporting request/block_request branches. Broader request semantics, whole-file WP_Http ownership, and installed distribution behavior remain later compiler-driven gates."
      }
    ],
    ...
    validation_result: {
      ...
      response_truncation_claimed: false,
      installed_wordpress_behavior_claimed: false,
  ...
  const receipt = {
    ...
    id: "receipt:wphx-312-71-wp-http-request-response-size-options-candidate",
    issue: ISSUE,
    recorded_at: RECORDED_AT,
    artifacts: [
      { path: OUT, role: "WP_Http request response-size options Haxe candidate manifest" },
      { path: OWNERSHIP, role: "ownership manifest for WP_Http request response-size options Haxe candidate" },
      { path: RUNNER, role: "deterministic PHP CLI oracle/candidate Haxe fixture generator" },
      { path: HXML, role: "Haxe compile target for request response-size options candidate" },
