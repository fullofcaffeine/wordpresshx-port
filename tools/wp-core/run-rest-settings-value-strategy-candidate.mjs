...
const ISSUE = {
  id: "wordpresshx-l76.10.4",
  external_ref: "WPHX-311.04",
  title: "Promote REST settings get-item value strategy to Haxe candidate"
};
const HXML = "fixtures/wp-core/rest-settings-value-strategy-candidate.hxml";
const OUT_ROOT = "build/wp-core/wphx-311-04";
const HAXE_OUT = `${OUT_ROOT}/haxe`;
const ORACLE_ROOT = `${OUT_ROOT}/oracle`;
const CANDIDATE_ROOT = `${OUT_ROOT}/candidate`;
const PROBE = `${OUT_ROOT}/probe.php`;
const OUT = "manifests/wp-core/wphx-311-04-rest-settings-value-strategy-candidate.v1.json";
const OWNERSHIP = "manifests/ownership/wphx-311-04-rest-settings-value-strategy-candidate.v1.json";
const RECEIPT = "receipts/wp-core/wphx-311-04-rest-settings-value-strategy-candidate.v1.json";
...
const RECORDED_AT = "2026-06-22T10:05:00.000Z";
...
const HAXE_SOURCES = [
  ...
  "fixtures/wp-core/src/wphx/fixtures/wp/core/RestSettingsValueStrategyCandidateEntry.hx"
];
...
const FIXTURE_CASES = [
  {
    id: "rest-settings-value:get-item-fallback",
    symbol: "WP_REST_Settings_Controller::get_item",
    focus: "null rest_pre_get_setting results fall back to get_option default values before schema preparation"
  },
  {
    id: "rest-settings-value:get-item-filter-override",
    symbol: "rest_pre_get_setting",
    focus: "non-null rest_pre_get_setting overrides skip option storage and still flow through schema preparation"
  },
  {
    id: "rest-settings-value:invalid-stored-value-null",
    symbol: "WP_REST_Settings_Controller::prepare_value",
    focus: "values that fail REST schema validation become null instead of being sanitized"
  },
  {
    id: "rest-settings-value:prepare-value-direct",
    symbol: "WP_REST_Settings_Controller::prepare_value",
    focus: "valid values are sanitized and invalid values become null through the typed Haxe decision route"
  }
];
...
function haxeBootstrapBlock(dirnameDepth) {
  return `if ( ! function_exists( 'wphx_311_04_bootstrap_haxe' ) ) {
\tfunction wphx_311_04_bootstrap_haxe() {
...
\t\t$wphx_311_04_lib = dirname( __DIR__, ${dirnameDepth} ) . '/haxe/lib';
\t\tset_include_path( get_include_path() . PATH_SEPARATOR . $wphx_311_04_lib );
...
\t$response       = array();
\t$value_strategy = '\\\\wphx\\\\wp\\\\rest\\\\RestSettingsValueStrategy';

\tforeach ( $options as $name => $args ) {
\t\t$response[ $name ] = apply_filters( 'rest_pre_get_setting', null, $name, $args );

\t\tif ( $value_strategy::shouldUseOptionFallback( is_null( $response[ $name ] ) ) ) {
...
\t$value_strategy   = '\\\\wphx\\\\wp\\\\rest\\\\RestSettingsValueStrategy';
\t$validation_error = is_wp_error( rest_validate_value_from_schema( $value, $schema ) );

\tif ( $value_strategy::shouldReturnNullForInvalidSchemaValue( $validation_error ) ) {
\t\treturn null;
\t}

\tif ( $value_strategy::shouldSanitizeSchemaValue( $validation_error ) ) {
...
$GLOBALS['wphx_311_04_php_errors'] = array();
set_error_handler(
...
global $wpdb;
$wpdb = new WPHX_311_04_Fake_WPDB();
...
class WPHX_311_04_Settings_Probe extends WP_REST_Settings_Controller {
\tpublic function prepare_value_public( $value, $schema ) {
\t\treturn $this->prepare_value( $value, $schema );
\t}
}

function wphx_311_04_scalar( $value ) {
\tif ( is_int( $value ) ) {
...
function wphx_311_04_value( $value ) {
\tif ( is_array( $value ) ) {
\t\t$entries = array();
\t\tforeach ( $value as $key => $entry_value ) {
\t\t\t$entries[] = array(
\t\t\t\t'key'   => wphx_311_04_scalar( $key ),
\t\t\t\t'value' => wphx_311_04_value( $entry_value ),
\t\t\t);
\t\t}
\t\treturn array(
\t\t\t'type'    => 'array',
...
\t\t\t'data'    => wphx_311_04_value( $value->get_error_data() ),
\t\t);
\t}
\tif ( is_object( $value ) ) {
\t\treturn array(
...
\t\t\t'properties' => wphx_311_04_value( get_object_vars( $value ) ),
\t\t);
\t}
\treturn wphx_311_04_scalar( $value );
}

function wphx_311_04_case( $id, $symbol, $value, $meta = array() ) {
\treturn array(
\t\t'id'     => $id,
\t\t'symbol' => $symbol,
\t\t'value'  => wphx_311_04_value( $value ),
\t\t'meta'   => wphx_311_04_value( $meta ),
\t);
}

function wphx_311_04_reset_state() {
...
function wphx_311_04_register_settings() {
\tregister_setting(
\t\t'wphx_rest_group',
\t\t'wphx_rest_text',
\t\tarray(
...
function wphx_311_04_seed_options() {
\tglobal $wpdb;
\t$wpdb->set_option( 'wphx_rest_text', ' stored text ', 'off' );
\t$wpdb->set_option( 'wphx_rest_named', '12', 'off' );
\t$wpdb->set_option( 'wphx_rest_object', 'not-an-object', 'off' );
}

function wphx_311_04_run_cases() {
\tglobal $wpdb;
\t$cases      = array();
\t$controller = new WPHX_311_04_Settings_Probe();
\t$request    = new WP_REST_Request( 'GET', '/wp/v2/settings' );

\twphx_311_04_reset_state();
\twphx_311_04_register_settings();
\twphx_311_04_seed_options();
\t$cases[] = wphx_311_04_case(
\t\t'rest-settings-value:get-item-fallback',
\t\t'WP_REST_Settings_Controller::get_item',
\t\t$controller->get_item( $request ),
\t\tarray( 'queries' => $wpdb->queries )
\t);
...
\t\tfunction ( $result, $name, $args ) use ( &$events ) {
\t\t\t$events[] = array(
\t\t\t\t'name'       => $name,
\t\t\t\t'optionName' => $args['option_name'],
\t\t\t\t'wasNull'    => is_null( $result ),
\t\t\t);
\t\t\tif ( 'wphx_rest_text' === $name ) {
...
\t);
\t$cases[] = wphx_311_04_case(
\t\t'rest-settings-value:get-item-filter-override',
\t\t'rest_pre_get_setting',
\t\t$controller->get_item( $request ),
\t\tarray(
\t\t\t'events'  => $events,
...
\t$cases[] = wphx_311_04_case(
\t\t'rest-settings-value:invalid-stored-value-null',
\t\t'WP_REST_Settings_Controller::prepare_value',
\t\tarray( 'objectValue' => $response['wphx_rest_object'] )
\t);

\t$cases[] = wphx_311_04_case(
\t\t'rest-settings-value:prepare-value-direct',
\t\t'WP_REST_Settings_Controller::prepare_value',
\t\tarray(
\t\t\t'validInteger'   => $controller->prepare_value_public( '42', array( 'type' => 'integer' ) ),
\t\t\t'invalidInteger' => $controller->prepare_value_public( 'nope', array( 'type' => 'integer' ) ),
\t\t\t'validObject'    => $controller->prepare_value_public(
\t\t\t\tarray( 'enabled' => true, 'label' => 'ok' ),
\t\t\t\tarray(
\t\t\t\t\t'type'       => 'object',
\t\t\t\t\t'properties' => array(
\t\t\t\t\t\t'enabled' => array( 'type' => 'boolean' ),
...
\t\t\t\t)
\t\t\t),
\t\t\t'invalidObject'  => $controller->prepare_value_public( 'not-object', array( 'type' => 'object' ) ),
\t\t)
\t);
...
$reflection = new ReflectionMethod( 'WP_REST_Settings_Controller', 'get_item' );
$snapshot   = array(
\t'mode'                  => $mode,
...
\t),
\t'haxeStrategyLoaded'    => class_exists( '\\\\wphx\\\\wp\\\\rest\\\\RestSettingsValueStrategy' ),
\t'cases'                 => wphx_311_04_run_cases(),
\t'phpErrors'             => $GLOBALS['wphx_311_04_php_errors'],
);
...
function writeOrCheck(path, contents) {
  if (checkOnly) {
    ...
    if (current !== contents) {
      throw new Error(`${path} is stale; run npm run wp:core:wphx-311-rest-settings-value-candidate`);
    }
    return;
  }
...
function ownershipManifest(manifestSha, upstreamDigest) {
  return {
    ...
    manifest_id: "ownership:wp-core/rest-settings-value-strategy-candidate",
    issue: {
      id: ISSUE.id,
    ...
    unit: {
      ...
      name: "WP_REST_Settings_Controller get_item value strategy",
      area: "wp-includes/rest-api/endpoints/class-wp-rest-settings-controller.php",
      public_contract:
        "The REST settings controller must preserve get_item and prepare_value PHP ABI while typed Haxe owns fallback, validation-null, and sanitize-route decisions."
    },
    ...
    owned_paths: [...HAXE_SOURCES, "tools/wp-core/run-rest-settings-value-strategy-candidate.mjs", OUT, OWNERSHIP, RECEIPT],
    generated_paths: [OUT, OWNERSHIP, RECEIPT, OUT_ROOT],
    abi_policy: {
      php_surface_preserved: [
        "WP_REST_Settings_Controller::get_item remains public",
        "WP_REST_Settings_Controller::prepare_value remains protected",
        "REST filters, get_option(), schema validation, and schema sanitization remain native WordPress/PHP calls"
      ],
      haxe_owned_decisions: [
        "rest_pre_get_setting null fallback route",
        "invalid schema values return null",
        "valid schema values flow to REST sanitization",
        "null sanitizer input preservation decision"
      ],
      native_boundaries: [
        "PHP array request/response ABI",
        "rest_pre_get_setting filter callbacks",
        "get_option() storage semantics",
        "rest_validate_value_from_schema() and rest_sanitize_value_from_schema()"
      ]
    },
    verification: {
      oracle_commands: [
        "npm run wp:core:wphx-311-rest-settings-value-candidate",
    ...
    notes:
      "This slice keeps native PHP arrays and WordPress schema functions at the boundary. update_item mutation routing remains a later WPHX-311 candidate."
  };
}
...
command("php", ["-l", `${HAXE_OUT}/lib/wphx/wp/rest/RestSettingsValueStrategy.php`]);
...
const manifest = {
  schema: "wphx.wp-core-rest-settings-value-strategy-candidate.v1",
  issue: ISSUE.external_ref,
  generated_at: RECORDED_AT,
  generator: "tools/wp-core/run-rest-settings-value-strategy-candidate.mjs",
  inputs: {
  ...
  fixture: {
    candidate_kind: "haxe_generated_rest_settings_value_strategy_shell",
    ...
    haxe_strategy_probe: {
      ...
      route: "typed_haxe_rest_settings_value_plan",
      owned_methods: ["get_item", "prepare_value"]
    },
    haxe_source_audits: haxeSourceAudits,
    source_escape_audit_passed: sourceEscapeAuditPassed,
    native_boundaries: [
      {
        id: "native-wordpress-value-functions",
        reason:
          "get_option(), rest_validate_value_from_schema(), and rest_sanitize_value_from_schema() remain native WordPress behavior authorities in this slice."
      },
      {
        id: "native-filter-boundary",
        reason:
          "rest_pre_get_setting remains a PHP plugin extension point and is deliberately not represented as a typed Haxe callback surface yet."
      }
    ],
  ...
  generated: {
    ...
    strategy_php: inputRecord(`${HAXE_OUT}/lib/wphx/wp/rest/RestSettingsValueStrategy.php`),
    php_lint: {
  ...
  remaining_gaps: [
    {
      id: "update-item-not-yet-haxe-owned",
      owner: "WPHX-311",
      detail:
        "This slice ports read-side settings value decisions. update_item pre-update/null-delete/error routing remains a later Haxe candidate."
    },
    {
...
const receipt = {
  ...
  id: "receipt:wphx-311-04-rest-settings-value-strategy-candidate",
  issue: ISSUE,
  recorded_at: RECORDED_AT,
  artifacts: [
    {
      path: OUT,
      role: "typed Haxe REST settings value strategy candidate manifest"
    },
    {
      path: OWNERSHIP,
    ...
    {
      path: "src/wphx/wp/rest/RestSettingsValueStrategy.hx",
      role: "typed Haxe REST settings value strategy source"
    },
    {
      path: "tools/wp-core/run-rest-settings-value-strategy-candidate.mjs",
